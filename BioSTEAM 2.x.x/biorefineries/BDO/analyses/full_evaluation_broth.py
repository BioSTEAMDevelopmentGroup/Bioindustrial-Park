...
from biorefineries.BDO.system_broth import (
    spec, BDO_sys, get_conc_aqueous_broth_MPSP, get_GWP, get_FEC, flowsheet, BDO_tea
)
from biorefineries.BDO.analyses import models_broth as models
...
model.evaluate(notify=50, autoload=False, 
               # autosave=20, file='unfinished_evaluation',
               )
...
file_to_save = 'BDO_broth_%s.%s.%s-%s.%s'%(dateTimeObj.year, dateTimeObj.month, dateTimeObj.day, dateTimeObj.hour, dateTimeObj.minute)
...
#         spec.load_specifications(spec_1=spec.spec_1, spec_2=spec.spec_2, spec_3=spec.spec_3)
#         MPSP = get_conc_aqueous_broth_MPSP()
#         MPSPs[i].append(MPSP)
