...
spearman_metrics = orgacids_model.metrics[0:6]
...
with pd.ExcelWriter('E1_baseline_evaluation_88.xlsx') as writer:
    ...
    orgacids_model.table.to_excel(writer, sheet_name='Raw data')
...
with pd.ExcelWriter('E2_across_IRR_pure.xlsx') as writer:
    ...
    orgacids_model_IRR.table.to_excel(writer, sheet_name='Raw data')
...
with pd.ExcelWriter('E3_across_lactic_acid_yield_88.xlsx') as writer:
    ...
    orgacids_model_LA_yield.table.to_excel(writer, sheet_name='Raw data')
...
with pd.ExcelWriter(name) as writer:
    ...
    orgacids_model_feedstock.table.to_excel(writer, sheet_name='Raw data')
...
from chaospy import distributions as shape
from orgacids.models import orgacids_model_SA_content, set_feedstock_succinic_acid_content

'''Evaluate'''
np.random.seed(3221)
N_simulation = 10 # 1000
SA_content_samples = orgacids_model_SA_content.sample(N=N_simulation, rule='L')
orgacids_model_SA_content.load_samples(SA_content_samples)

SA_content_coordinate = np.linspace(0, 0.05, 5+1) # step = 0.01
...
SA_content_data = orgacids_model_SA_content.evaluate_across_coordinate(
...
with pd.ExcelWriter('E5_across_feedstock_succinic_acid_content_88.xlsx') as writer:
    ...
    orgacids_model_SA_content.table.to_excel(writer, sheet_name='Raw data')
