...
def get_installed_cost(system):
    return lambda: sum(i.installed_cost for i in orgacids_sub_sys[system])/1e6
...
check_installed_cost = \
    lambda: sum(get_installed_cost(system)() 
                for system in orgacids_sub_sys.keys()) - orgacids_tea.installed_cost/1e6
...
check_heating_demand = \
    lambda: sum((get_heating_demand(system)() for system in orgacids_sub_sys.keys()), 
...
check_cooling_demand = \
    lambda: sum(get_cooling_demand(system)() for system in orgacids_sub_sys.keys())
...
get_system_utility_cost = lambda: orgacids_tea.utility_cost/1e6
...
orgacids_model_SA_content = Model(orgacids_sys, metrics)
orgacids_model_SA_content.set_parameters(parameters)
