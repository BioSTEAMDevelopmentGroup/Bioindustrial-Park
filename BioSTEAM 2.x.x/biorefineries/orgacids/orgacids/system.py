...
def adjsut_F402_V():
    # SA_percent = feedstock.imass['SuccinicAcid']/(feedstock.F_mass - feedstock.imass['H2O'])
    H2O_molfrac = D404_P.outs[0].imol['H2O']/D404_P.outs[0].F_mol
    # V = H2O_molfrac + (SA_percent/5)
    V = H2O_molfrac
    F402.V = V
    F402._run()

F402 = bst.units.Flash('F402', ins=D404_P-0, V=0.9, P=101325)
F402.specification = adjsut_F402_V

F402_H = bst.HXutility('F402_H', ins=F402-0, outs=3-R403, V=0, rigorous=True)
F402_P = units.OrganicAcidsPump('F402_P', ins=F402-1)


def adjust_D405_Lr(Lr):
    D405.Lr = Lr
    D405._run()
    product = D405.outs[1]
    LA_pur = product.imass['LacticAcid'] / product.F_mass
    return LA_pur - 0.881
...
D405 = bst.units.ShortcutColumn('D405', ins=F402_P-0, outs=('D405_g', 'D405_l_LA'),
                                ...
                                is_divided=True,
                                product_specification_format='Recovery',
                                # Lr=0.99, Hr=0.99, k=1.2) # 0% SA, 99.94% purity, F402.V=0.9
                                Lr=0.05, Hr=0.9995, k=1.2) # 0% SA -> 88.40%; 5% SA -> 88.09% purity, F402.V adjusted
                                # Lr=0.001, Hr=0.9995, k=1.2, Rmin=0.05) # 0% SA -> 88.40%; 5% SA -> 88.09% purity, F402.V adjusted
                                
D405.specification = bst.process_tools.BoundedNumericalSpecification(adjust_D405_Lr,\
  # 0.005, 0.56, solver = flx.IQ_interpolation, xtol=0.0001, ytol=0.02)
  # 0.001, 0.999, solver = flx.IQ_interpolation, xtol=0.001, ytol =0.02)
  0.001, 0.999, solver = flx.IQ_interpolation, xtol=0.00001, ytol =0.001)

D405_H1 = bst.units.HXutility('D405_H1', ins=D405-0, V=0, rigorous=True)
# Cool the final product down for storage
...
# FGD lime used in BT not included here for sizing, but that is very trivial
# (<0.5%) of total used lime
T604 = units.LimeStorageBin('T604', ins=lime_fresh, outs='lime')
T604.line = 'Lime storage tank'
T604_P = bst.units.ConveyingBelt('T604_P', ins=T604-0, outs=fermentation_lime)
