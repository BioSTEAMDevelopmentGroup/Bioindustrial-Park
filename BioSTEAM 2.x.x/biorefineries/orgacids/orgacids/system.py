...
from biosteam import main_flowsheet as m_f
...
# Separate out water and more volatile components
F401 = bst.units.Flash('F401', ins=S402-1, outs=('vapor', 'liquid'),
                        T=379, P=101325)

# Condense waste vapor for wastewater treatment, temperature selected based on Ethanol.Tb
H401 = units.WasteVaporCondenser('H401', ins=F401-0, T=345, V=0)
...
# LA + EtOH --> EtLA + H2O
# R402.ins[1] & [4] reserved for ethanol recycle; [3] for supp_ethanol; [2] for LA recycle;
# R402.ins[0] is ~50% w/w conc. LA feed
...
# Split_S403 takes in LA recycle stream -- not the EtLA-rich product stream -- and 
# discards a small fraction of it to prevent accumulation. 
# It might have been a better idea to send this to M401 instead, but currently not 
# possible to simulate this recycle stream
Split_S403 = bst.units.Splitter('Split_S403',ins = S403-1, outs = ('bottom1', 'bottom2'), split = 0.97)

# LA recycle to esterification
Split_S403-0-2-R402

# Condense reactants into liquid phase, temperature selected based on Ethanol.Tb
H402 = bst.units.HXutility('H402', ins=S403-0, T=345, V=0)

# EtLA + H2O --> LA + EtOH
# R403.ins[0] is main EtLA feed; [1] is supplementary water (almost never needed); 
# [2] & [3] are recycled water (with some EtLA and LA)
# R403.outs[1] is discarded fraction of recycled water 
...
F_pre_S404 = bst.Flash('F_pre_S404', ins=pre_S404-1, V = 0.8, P=101325)
# F_pre_S404 = bst.units.MultiEffectEvaporator('F_pre_S404', ins=pre_S404-1, outs=('vapor', 'liquid'),
...
S404 = bst.units.ShortcutColumn('S404', ins=F_pre_S404-1, outs=('vapor', 'LA'),
                                LHK=('EthylLactate', 'LacticAcid'),
                                is_divided=True,
                                product_specification_format='Recovery',
...
BlowdownMixer = bst.units.BlowdownMixer('BlowdownMixer', ins = (CT.outs[1], BT.outs[2]))

# BlowdownMixer-0-7-M501
M501.ins[-1] = BlowdownMixer.outs[0]
...
# orgacids_sys0 = System('orgacids_sys0',
#     [U101,
#      T201,
#      M201,
#      M202,
...
#      P601,
#      FWT],
#     facilities=(BT, CT, PWC, CIP, ADP, BlowdownMixer))


HXN = facilities.HX_Network(ID = 'HXN')

orgacids_sys = m_f.create_system('orgacids_sys', feeds = [i for i in m_f.stream if i.sink and not i.source])
...
#     [orgacids_sys0],
#     facilities=(HXN, BT, CT,))

# orgacids_sys.path = [orgacids_sys0.simulate]
