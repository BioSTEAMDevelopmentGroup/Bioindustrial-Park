...
species_IDs = [
        ...
        #!!! Z_mobilis needs to be changed to S_cerevisiae
        'Z_mobilis', 'T_reesei',
...
addspecies('H2O', 'Ethanol', 'AceticAcid', 'Furfural', 'Glycerol',
           ...
           'CitricAcid', 'LacticAcid', 'CisCis-MuconicAcid', 'PropionicAcid',
           'SuccinicAcid'
#          , 'CaCO3', 'CalciumLactate'
           )

# Amend organic acid properties based on CRC Handbook (Haynes et al., 97th Edn.)
sp.HydroxypropionicAcid.phase = 'l'
sp.LacticAcid.Tb = 122 + 273.15
sp.LacticAcid.H_int_l_T_ref_l_to_Tb = sp.LacticAcid.H
sp.LacticAcid.Hvap_Tbm = sp.LacticAcid.Hvap
sp.LacticAcid.Hf = -163122*cal2joule # from Yoel
...
# Other species
# Z. mobilis is the species used for fermentation
addsubstance('Z_mobilis', MW=24.6265, Hf=-31169.39*cal2joule)
copy('WWTsludge', 'Z_mobilis')
# T. reesei is the species used to produce cellulase enzyme for saccharification (enzymatic hydrolysis)
...
addsubstance('CSL',
             MW=sp.Protein.MW/4+sp.H2O.MW/2+sp.LacticAcid.MW/4, 
             Hf=sp.Protein.Hf/4+sp.H2O.Hf/2+sp.LacticAcid.Hf/4)
# Placeholder for chemicals used in separation system
# Now MW and Hf comes from CaSO4, MW is from thermo.Chemical
# Hf from CRC Handbook (Haynes et al., 97th Edn.)
addsubstance('SeparationChemicals', MW=136.1406, Hf = -1434.5e3)
