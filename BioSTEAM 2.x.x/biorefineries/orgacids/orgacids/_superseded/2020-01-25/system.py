...
'''
TODO:
    SEARCH FOR #??? FOR QUESTIONS, CHECK NUMBERS WITH HUMBIRD
...
S301 = bst.units.InvSplitter('S301', ins='CSL', outs=(CSL1, CSL2))
S302 = bst.units.InvSplitter('S302', ins='DAP', outs=(DAP1, DAP2))
...
R301 = units.SaccharificationAndCoFermentation('R301', ins=(M302-0, CSL1, DAP1))
M303 = bst.Mixer('M303', ins=(R301-2, CSL2, DAP2))
...
CSL_index = species.index('CSL')
DAP_index = species.index('DAP')
def update_CSL1_and_DAP1_loading():
    cooled_hydrolysate_massnet = H301.outs[0].massnet
    # 0.25 wt% based on Humbird et al.
    CSL1.mass[CSL_index] = 0.25/100 * cooled_hydrolysate_massnet
    # 0.33 g/L based on Humbird et al.
    DAP1.mass[DAP_index] = 0.33/1000 * cooled_hydrolysate_massnet

# To M303 then R302 seed train
CSL2_over_saccharified_slurry = CSL2.mol/42607 # Total flow of 304 in Humbird et al.
DAP2_over_saccharified_slurry = DAP2.mol/42607 
def update_CSL2_and_DAP2_loading():
    saccharified_slurry_mass = R301.outs[2].massnet
    CSL2.mol[:] = CSL2_over_saccharified_slurry * saccharified_slurry_mass
    DAP2.mol[:] = DAP2_over_saccharified_slurry * saccharified_slurry_mass

fermentation_sys = System('fermentation_sys',
                   network=(H301,
                            update_cellulase_loading,
                            update_CSL1_and_DAP1_loading,
                            M301, M302, R301,
                            update_CSL2_and_DAP2_loading,
...
#??? recycled_water from where? This stream needs to be fixed
...
R601 = units.AnaerobicDigestion('R601', ins=(WWT_cost-0, well_water_in),
...
CSL_fresh = Stream('DAP_fresh')
CSL_fresh.copylike(S301.ins[0])
DAP_fresh = Stream('CSL_fresh')
DAP_fresh.copylike(S302.ins[0])
...
CIP_chems_out = Stream('CIP_chems_out', species=substance)
CIP_chems_out.copyflow(CIP_chems_in)
plant_air_in = Stream('plant_air_in', species=substance, flow=(83333,))
...
T703 = units.CSLStorageTank('T703', ins=CSL_fresh, outs=0-S301)
T703.line = 'CSL storage tank'
T704 = units.DAPStorageTank('T704', ins=DAP_fresh, outs=0-S302)
T704.line = 'DAP storage tank'
...
PWC = bst.facilities.ProcessWaterCenter('PWC',
                                        ...
                                        outs=PWC_water_out)
S701 = bst.units.InvSplitter('S701', ins=PWC_water_out, 
...
def update_water_loss():
    process_water.mol[water_index] = sum([i[j] for i, j in process_water_data])
    
def update_discharged_water():
    if makeup_water.massnet == 0:
        discharged_water.mol[water_index] = \
            S604.outs[0].mol[water_index] - process_water.mol[water_index]
