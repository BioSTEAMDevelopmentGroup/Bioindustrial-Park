...
'''
TODO:
   ADD SEPARATION UNITS
   FOR FERMENTATION, CONSIDER:
       TEMPERATURE
...
class SeedTrain(Unit):
    ...
    def _run(self):
        ...
        EtOH = 0
        loss = 0
        if effluent.mass[effluent.index('Z_mobilis')] == 0:
                effluent.mass[effluent.index('Z_mobilis')] = self.X_init * \
                    effluent.volnet
        for i in range(0, 5):
            ...
            loss += simulated_flow[5]
        # Assume all lost glucose and xylose changed to CO2
        effluent.mass[effluent.index('Ethanol')] = feed.mass[effluent.index('Ethanol')] + EtOH
...
#??? # of recirculation pump is 5, yet the cost has not been divided by 5
# Change of cost of reciruculation pumps to be 47200/5
...
class SaccharificationAndCoFermentation(Unit):
    ...
    def __init__(self, ID='', ins=None, outs=(), P=101325):
        ...
        self.CSL_DAP_2constituents = ParallelRxn([
            #   Reaction definition                             Reactant    Conversion
    ...
    def _run(self):
        ...
        effluent.mass[effluent.index('Glucose')] = simulated_flow[2]
        effluent.mass[effluent.index('Xylose')] = simulated_flow[3]
        effluent.mass[effluent.index('Ethanol')] = simulated_flow[4]
        # Assume all lost glucose and xylose changed to CO2
        effluent.mass[effluent.index('CO2')] = \
            feed.mass[effluent.index('CO2')] + simulated_flow[5]
        self.CSL_DAP_2constituents(effluent.mass)
        vent.copyflow(effluent, ('CO2', 'NH3', 'O2'), remove=True)
...
# Costs scaled up to be ~ the 22e6 in Humbird et al.

# Placeholder copying the AmmoniaAdditionTank unit from cornstover biorefinery
@cost(basis='Flow rate', ID='Tank', units='kg/hr',
      S=410369, ub=False, CE=522, cost=236000*10, n=0.7, kW=0, BM=2)
@cost(basis='Flow rate', ID='Agitator', units='kg/hr',
      S=410369, ub=False, CE=522, cost=21900*2, n=0.5, kW=7.457, BM=1.5)
...
@cost(basis='Flow rate', ID='Tank', units='kg/hr',
      S=425878, ub=False, CE=522, cost=636000*10, n=0.7, kW=0, BM=1.8)
@cost(basis='Flow rate', ID='Pump', units='kg/hr',
      S=425878, ub=False, CE=522, cost=26800*10, n=0.8, kW=93.2125, BM=2.3)
@cost(basis='Flow rate', ID='Agitator', units='kg/hr',
      S=425878, ub=False, CE=522, cost=68300*10, n=0.5, kW=14.914, BM=1.5)
...
@cost(basis='Duty', ID='Heat exchangers', units='Gcal/hr',
      S=5*_Gcal2kJ, ub=False, CE=522, cost=23900*10, n=0.7, BM=2.2)
...
class AnaerobicDigestion(bst.Unit):
    ...
    **outs**
    ...
    def _run(self):
        feed, water_in = self.ins
        biogas, waste, sludge, water_out = self.outs
        biogas.phase = 'g'
        water_out.link = water_in
        biogas.T = waste.T = sludge.T = T = 35+273.15
        # Now the water is just to maintain heat balance
        water_out.T = feed.T
        water_in.mol[:] *= (feed.H - feed.H_at(T=T))/(water_out.H - water_in.H)
        while water_in.molnet < 0:
            water_out.T -= 1
            water_in.mol[:] *= (feed.H - feed.H_at(T=T))/(water_out.H - water_in.H)
...
class CIPpackage(bst.Facility):
    ...
    #     chems_out = bst.Stream(species=substance)
    #     chems_out.copylike(chems_in)
    #     self.outs[0] = chems_out
