...
def EB_simulate(feed, tau):
    ...
    X_i, P_i, S_g_i, S_x_i = feed.indices(['Z_mobilis',
                                           'LacticAcid',
                                           'Glucose',
                                           'Xylose'])
    mass_in = feed.mass
    volnet = feed.volnet
    concentration_in = mass_in/volnet
    X0, P0, S_g0, S_x0 = (concentration_in[i] for i in (X_i, P_i, S_g_i, S_x_i))
    ...
    Cf = [Xf, Pf, S_gf, S_xf]
    # Assume glucose fermentation led to 1:1 EtOH:LA on mass basis
    EtOH = (S_g0 - S_gf)* Y_PS_g * EtOH_over_LA
    loss = sum(C0) - sum(Cf) - EtOH
    Cf.append(EtOH)
    Cf.append(loss)
    mass_out = tuple([i * volnet for i in Cf])
    return mass_out
