...
'''Full evaluation'''
# orgacids_model.evaluate(simulation_number=3)
orgacids_model.evaluate(simulation_number=1)
...
LA_yield_coordinate = np.linspace(0.55, 0.93, 19+1) # step = 0.02
...
feedstock_carbs_coordinate = np.linspace(0.4, 0.7, 15+1) # step = 0.02

feedstock_data = orgacids_model_feedstock.evaluate_across_coordinate(
    'Feedstock carbohydate content', set_feedstock_carbs, 
    feedstock_carbs_coordinate, notify=True, simulation_number=3)
...
orgacids_model_IRR.evaluate(simulation_number=1)
# orgacids_model_IRR.evaluate(simulation_number=3)
