...
# # Modified from bst.units.Flash, add agitator	
# @cost(basis='Flow rate', ID='Agitator', units='kg/hr',	
#       kW=170, cost=90000, S=252891, CE=521.9, n=0.5, BM=1.5)	
...
class Reactor(Unit, PressureVessel, isabstract=True):
    ...
    kW_per_m3=0.985: float
        Power usage of agitator (0.985 converted from 5 hp/1000 gal as in [1]).        
    ...
    def compute_height_and_diameter_from_volume(self, V, ratio):
        logD = 1/3*log(4*V/pi/ratio, 10)
        D = 10**logD * 3.28084 # convert from m to ft
        L = ratio * D # ft
        return D, L
    ...
    def _design(self):
        ...
        if N == 0:
            ...
            V_reactor = V_total / N
            D, L = self.compute_height_and_diameter_from_volume(
                V_reactor, self.length_to_diameter)
...
class AcidulationReactor(Reactor):
    ...
    def _run(self):
        rxn = self.acidulation_rxns = ParallelRxn([
        #   Reaction definition                                           Reactant         Conversion
        ...
        acid_index = feed.chemicals.index('H2SO4')
        reactant_indices = rxn.chemicals.indices(rxn.reactants)
        needed_acid = 0
        for i in range(len(reactant_indices)):
            index = reactant_indices[i]
            needed_acid += -(rxn.stoichiometry[i][acid_index]) * feed.mol[index]
        ...
        rxn.adiabatic_reaction(effluent)
...
class Esterification(Reactor):
    ...
    'Amberlyst-15' required and consumes it.
    ...
    def __init__(self, ID='', ins=None, outs=(), thermo=None, *, 
                 ...
                 cat_load=0.039,  X1=None, X2=None, ethanol2acids=1.2, 
        ...
        self.length_to_diameter = length_to_diameter
        self.kW_per_m3 = kW_per_m3
        self.cat_load = cat_load
        self.X1, self.X2, self._tau = X1, X2, tau
        self.ethanol2acids = ethanol2acids
    ...
    def compute_X1_and_tau(self, mixed_stream, time_step):
        ...
        time_max = self.tau_max * 60 # tau_max in hr but time_step in min
    ...
    def _run(self):
        ...
        if self.assumeX2equalsX1:
            if self.X2 != self.X1 and self.X2 is not None:
                raise AttributeError('X1 and X2 different, but assumeX2equalsX1 is True')
            X2 = self.X2 = X1
        else:
            if not self.X2:
...
# =============================================================================
# Old Esterification code below, can compare results it wanted
# Yalin will remove those codes after Sarang reviews them
# =============================================================================
...
class HydrolysisReactor(Reactor):
    ...
    def _run(self):
        ...
        hydrolysis_rxns = self.hydrolysis_rxns = ParallelRxn([
        #   Reaction definition                                       Reactant   Conversion
