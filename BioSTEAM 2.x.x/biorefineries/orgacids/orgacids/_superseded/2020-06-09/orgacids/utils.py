...
def compute_extra_chemical2(feed, recycle, reactants_ID, chemical_ID, ratio):
    reactants_in_feed = feed.imol[reactants_ID].sum()
    reactants_in_recycle = recycle.imol[reactants_ID].sum()
    chemical_needed = ratio * (reactants_in_feed+reactants_in_recycle)
    chemical_extra = (feed.imol[chemical_ID]+recycle.imol[chemical_ID]) - chemical_needed
    return chemical_extra

def adjust_recycle2(feed, recycle, reactants_ID, chemical_ID, ratio):
    feed_chemical_needed = feed.imol[reactants_ID].sum()*ratio \
        - feed.imol[chemical_ID]
    
    recycle_chemical_extra = \
        recycle.imol[chemical_ID] - recycle.imol[reactants_ID].sum()*ratio
