...
#!!! Need to make sure Hfus and Psat are fine
# NIST https://webbook.nist.gov/cgi/cbook.cgi?ID=C67470&Mask=4, accessed 04/24/2020
chems.HMF.Hfus = 19800
# #!!! Also need to check Psat (cannot use not chemical.Psat to check)
# for chemical in chems:
#     if chemical.ID in phase_change_chemicals and not chemical.Hfus:
#         print(chemical.ID)
chems.HMF.copy_models_from(chems.Furfural, ['Psat'])
...
chems.Mannan.copy_models_from(chems.Glucan)

chems.Mannose.copy_models_from(chems.Glucose)
chems.Galactose.copy_models_from(chems.Glucose)
chems.Arabinose.copy_models_from(chems.Xylose)
...
chems.CaSO4.Cn.move_up_model_priority('Constant', 0) 
chems.Xylan.Cn.add_model(chems.Glucan.Cn)
chems.Arabinan.Cn.add_model(chems.Glucan.Cn)
...
for chemical in chems:
    if not chemical.formula: pass
        ...
        if not chemical.combustion:
            chemical.combustion = get_stoichiometry(chemical.atoms)
        if not chemical.HHV:
            if chemical.Hf:
                chemical.HHV = get_HHV_stoichiometry(chemical.combustion, 
                                                     chemical.Hf)
            else: 
                chemical.HHV = get_HHV_Dulong(chemical.atoms, chemical.MW)
        if not chemical.LHV:
            try: N_H2O = chemical.atoms['H']/2
            except: N_H2O = 0
