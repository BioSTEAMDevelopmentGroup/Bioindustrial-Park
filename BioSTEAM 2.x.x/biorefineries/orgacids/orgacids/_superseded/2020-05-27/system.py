...
pretreatment_feedstock_water = Stream('pretreatment_feedstock_water',
                                      T=95+273.15, P=4.7*101325, units='kg/hr')
# Flow is adjusted in T201, SulfuricAcidAdditionTank
...
T203 = units.OligomerConversionTank('T203', ins=R201-1)
F201 = units.OrganicAcidsFlash('F201', ins=T203-0, P=101325, Q=0)
...
#                                  T=371, V=0)
H201 = bst.units.HXutility('H201', ins=M204-0, V=0)
...
S401 = units.CellMassFilter('S401', ins=M401-0, outs=('cell_mass', ''),
...
#!!! Consider changing all Flash (and maybe BinaryDistillation, ShortcutColumn, etc)
# to OrganicAcids ones so that agitators and pumps can be added

# F401 = bst.units.Flash('F401', ins=S402-1, T=379, P=101325)
F401 = bst.units.Flash('F401', ins=S402-1, T=385, P=101325)
# F401 = units.OrganicAcidsFlash('F401', ins=S402-1, T=379, P=101325)

# Condense waste vapor for wastewater treatment
H401 = bst.units.HXutility('H401', ins=F401-0, V=0)
...
# broth, recycled ethanol 1, recycled LA, supplementary ethanol, recycled ethanol 2
#!!! Need to add catalyst for the esterification reaction
R402 = units.Esterification('R402', ins=(S4ex-1, '', '', separation_spp_ethanol, ''))

# Separate out Ethanol for recycling
# pre_S403 = bst.units.Flash('pre_S403', ins=R402-0, outs=('vapor', 'liquid'),
#                                 V = 0.6, P = 101325)
pre_S403 = bst.units.BinaryDistillation('pre_S403', ins=R402-0,
                                LHK=('Ethanol', 'H2O'),
                                is_divided=True,
                                product_specification_format='Recovery',
                                Lr = 0.99, Hr = 0.6, k=1.2)
# Condense Ethanol
H404 = bst.units.HXutility('H404', ins=pre_S403-0, outs=R402.ins[1], V=0)
# Distillation to separate esters from bottom
S403 = bst.units.BinaryDistillation('S403', ins=pre_S403-1,
                                    # outs=('', R402.ins[2]),
...
#!!! Split_S403 is implemented to avoid accumulating unwanted products, can this be removed?
Split_S403 = bst.units.Splitter('Split_S403',ins=S403-1, 
                                outs=(R402.ins[2], ''), split = 0.95)

# Condense reactants into liquid phase
H402 = bst.units.HXutility('H402', ins=S403-0, V=0)
...
pre_S404 = bst.units.ShortcutColumn('pre_S404', R403-0,
                                LHK=('Ethanol', 'H2O'),
                                product_specification_format='Recovery',
                                is_divided=True,
                                Lr=0.9, Hr=0.9995, k=1.2)
# Condense waste vapors
H403 = bst.units.HXutility('H403', ins=pre_S404-0, outs=R402.ins[4], V=0)
F_pre_S404 = bst.Flash('F_pre_S404', ins=pre_S404-1, V=0.8, P=101325)

# MultiEffectEvaporator giving /0 error; appears to split vapor to [1] and liquid to [0]
H_pre_S404 = bst.HXutility('H_pre_S404', ins=F_pre_S404-0, outs=R403.ins[3], V=0)

# To get the final acid product
# Purity = 98% needs Lr = 0.9995, Hr = 0.999
# Purity = 80% needs Lr=0.9948, Hr=0.99
S404 = bst.units.ShortcutColumn('S404', ins=F_pre_S404-1,
                                LHK=('EthylLactate', 'LacticAcid'),
                                is_divided=True,
                                product_specification_format='Recovery',
                                Lr=0.9995, Hr=0.999, k=1.2)
# F_S404 = bst.Flash('F_S404', ins=S404-0, V=0.8, P=101325)

H_S404 = bst.units.HXutility('H_S404', ins=S404-0, V=0)
...
# Flow is adjusted in AnaerobicDigestion
# well_water_in = Stream('well_water_in', Water=147140*plant_size_ratio, T=13+273.15)
well_water_in = Stream('well_water_in', T=13+273.15)
...
PS_stripping_water = bst.units.ProcessSpecification(
    'PS_stripping_water', ins=stripping_water, specification=update_stripping_water)
...
M501 = bst.units.Mixer('M501', ins=(H201-0, S4ex-0, 
                                    Split_S403-1, 
                                    R402-1, R403-1, 
                                    U501-1, H_S404-0, '', '', ''))
...
#!!! Keep biosteam native Tank setting consistent
T605 = units.OrganicAcidsStorageTank('T605', ins=ethanol_fresh,
                                     tau=7*24, V_wf=0.9,
                                     vessel_type='Floating roof',
                                     vessel_material='Carbon steel')
T605_Pump = units.OrganicAcidsPump('T605_Pump', ins=T605-0,
...
orgacids_sys = System('orgacids_sys',
    [U101,
     ...
     System('esterification_recycle',
        [System('esterification_outer_loop',
            [System('esterification_inner_loop',
                [R402, pre_S403, H404], recycle=H404-0),
             # S403],
              S403, Split_S403],
             # recycle=S403-0),
         ...
         System('hydrolysis_recycle',
                [R403, pre_S404, F_pre_S404, H_pre_S404],
     ...
     System('wastewater_treatment_loop',
        [M502, R502, S501, S502, M504, S503, M503],
        recycle=M503-0),
     M505,
     S504,
     H401,
     M204,
     H201],
    facilities=(S601, T601, T602, T603, S602, T604, 
                T605,
                T605_Pump,
                P602, T606,
# T605, P601, T602, T603, T604, S602,
#                 PS_separation_sulfuric_acid, S601,
#                 P602, T606, T601,
                BT, CT, PWC, CIP, ADP, FWT))
...
orgacids_sys_no_boiler_tea = OrgacidsTEA(
        ...
        OSBL_units=(WWT_cost, T601, T602, T603, T604, T605, 
                    T605_Pump, 
                    T606, P602,
                    CT, PWC, CIP, ADP, FWT),
...
orgacids_sub_sys = {
    'feedstock_sys': (U101,),
    'pretreatment_sys': (T201, M201, M202, M203, R201, 
                         # T202, 
                         T203, F201, M204, 
    ...
    'other_facilities': (CIP, ADP, FWT, PWC,
                         S601, T601, T602, T603, S602, T604, T605, T605_Pump,
                         P602, T606)
    }
