...
from biosteam.units import Flash, Mixer, Pump, SolidsSeparator, StorageTank
...
class AcidulationReactor(Unit):
    ...
    def __init__(self, ID='', ins=None, outs=()):
        ...
        # self.acidulation_rxns.basis = 'mol'
        # self.H_rxns = 0
            
    def _run(self):
        feed, acid = self.ins
        effluent = self.outs[0]
        # #!!! Not considering H_rxn for consistency
        # H_rxns = self.H_rxns
        ...
        # print(f'H before reaction is {effluent.H}')
        # self.acidulation_rxns.adiabatic_reaction(effluent.mol)
        self.acidulation_rxns.adiabatic_reaction(effluent)
        # print(f'H after reaction is {effluent.H}')
        # effluent.H += H_rxns
        # print(f'H_rxns is {H_rxns}')
        # print(f'H after considering H_rxns is {effluent.H}')
...
class AnaerobicDigestion(Unit):
    ...
    **outs**
    ...
    def __init__(self, ID='', ins=None, outs=(), *, reactants, split=()):
        Unit.__init__(self, ID, ins, outs)
    ...
    @staticmethod
    def _well_water_usage(cool_water_mol, feed, cool_water, hot_water):
        mol = hot_water.imol['Water'] = cool_water_mol
        H_at_35C = feed.thermo.mixture.H(mol=feed.mol, phase='l', T=35+273.15, P=101325)
        mol_new = (feed.H - H_at_35C)/(hot_water.H - cool_water.H)
        return mol - mol_new
        
    def _run(self):
        feed, cool_water = self.ins
        biogas, treated_water, sludge, hot_water = self.outs
        biogas.phase = 'g'
        hot_water.link_with(cool_water, TP=False)
        biogas.T = treated_water.T = sludge.T = hot_water.T = 35+273.15

        # # Water flow is adjusted to maintain heat balance
        cool_water_mol = cool_water.imol['Water']
        # Reset flow to avoid divide by 0 error, 147140 based on stream 903 in Humbird et al.
        cool_water_mol = 147140
        cool_water_mol = aitken_secant(self._well_water_usage,
                                       cool_water_mol, cool_water_mol+1, 
                                       1, 1,
                                       args=(feed, cool_water, hot_water),
                                       maxiter=1000)
        # H_at_35C = feed.thermo.mixture.H(mol=feed.mol, phase='l', T=35+273.15, P=101325)
...
# Ethanol storage tank, actually not very good to scale down from a big tank/pump
...
# bst.units.StorageTank won't simulate for 0 flow 
...
# bst.units.Pump won't simulate for 0 flow 
