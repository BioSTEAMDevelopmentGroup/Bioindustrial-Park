...
# Separate out target acid
#!!! Need to use real numbers, or reasonable assumptions for K_specs1 and other parameters
#!!! Need to add duty
K_specs1 = np.random.random_sample((len(S402.outs[1].chemicals),))
# outs[0] is vapor, outs[1] is liquid
S403 = units.MultiComponentDistillation('S403', ins=S402-1,
                                         LHK = ['AceticAcid', 'LacticAcid'],
                                        rLKD=0.98, rHKB=0.98, K_HK=0.5,
                                        K_specs=K_specs1
                                        )
#!!! Need to consider Methanol recycled from the hydrolysis reactor
#!!! No need to consider HX?
# Need to find methanol cost
R402 = units.EsterificationReactor('R402', ins=(S403-0, methanol))
#??? Further purification?
#!!! Need to differentiate top from bottom
#!!! Need to use real numbers, or reasonable assumptions for K_specs2 and other parameters
#!!! Need to add duty
K_specs2 = np.random.random_sample((len(R402.outs[0].chemicals),))
S404 = units.MultiComponentDistillation('S404', ins=R402-0,
                                        LHK = ['AceticAcid', 'MethylLactate'],
                                        rLKD=0.98, rHKB=0.98, K_HK=0.5,
                                        K_specs=K_specs2
                                        )
#!!! Need to add acid to catalyze the hydrolysis step?
R403 = units.HydrolysisReactor('R403', ins=S404-0)
...
M501 = bst.units.Mixer('M501', ins=(H201-0, S403-1, S404-1, '', ''))
...
P601 = bst.units.Pump('P601', ins=R403-0)
...
#!!! Need to consider pretreatment and separation acid water
...
process_water_streams = (WWT_caustic, stripping_water, warm_process_water, steam,
                         pretreatment_acid_water, separation_acid_water,
                         BT.outs[1], CT.outs[1])
