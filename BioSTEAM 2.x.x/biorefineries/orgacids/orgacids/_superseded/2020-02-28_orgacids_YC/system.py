...
from biosteam import Distillation
...
orgacids_chemicals.Methanol.Hc = 715e3
orgacids_chemicals.MethylAcetate.Hc = 1584337
orgacids_chemicals.refresh_constants()
...
#!!! This first distillation step is probably not needed
#!!! Now all water goes to vapor because of the high Tb of LacticAcid...

def special_flash_run():
    # Unpack
    vap, liq = S403.outs
    feed = S403.ins[0]

    # Vapor Liquid Equilibrium
    ms = S403._multistream
    ms.empty()
    vle_chemicals = ('Water', 'AceticAcid')
    ms.imol['l', vle_chemicals] = feed.imol[vle_chemicals]
    ms.vle(P=S403.P, V=S403.V)
...
S403 = bst.Flash('S403', ins=S402-1, V=0.95, P=101325)
S403._run = special_flash_run
...
S404 = Distillation('S404', ins=R402-0,
                    ...
                    Lr=0.9, Hr=0.9, k=1.2)
run_S404 = S404._run
def special_run_S404():
    run_S404()
    #: TODO: Set better temperature
    S404._boilup_bubble_point.T = 419
    S404.outs[1].T = 419
S404._run = special_run_S404
...
def update_FGD_lime_boilerchems_and_ash_loadings():
    ...
    ash.imol['Substance'] = (emission_ash + FGD_lime.imol['Lime']) * 1.18 
    boilerchems.imol['Substance'] = 0.24620/865 * FGD_lime.imol['Lime']
