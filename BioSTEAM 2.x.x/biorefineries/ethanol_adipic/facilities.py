...
from ethanol_adipic.utils import CEPCI
...
class CT(Facility):
    """
    Create a cooling tower process with capital cost and power based on the flow rate 
    ...
    def _run(self):
        ...
        cooling_water_utilities = self.cooling_water_utilities = {}
        ...
        for u in self.system.units:
            ...
            if hasattr(u, 'heat_utilities'):
                for hu in u.heat_utilities:
                    if hu.agent is agent:
                        cooling_water_utilities[f'{u.ID} - {hu.ID}'] = hu
                        total_duty -= hu.duty
        
        hu_cooling = self.heat_utilities[0]
        hu_cooling.mix_from(cooling_water_utilities.values())
        hu_cooling.reverse()        
...
# # No heat recovery included in design
# @cost(basis='Duty', ID='Heat recovery', units='Gcal/hr',
#       cost=41000, S=-2, CE=CEPCI[2009], n=0.7, BM=2.2)
class CHP(Facility):
    """
    ...
    def _design(self):
        ...
        lime.imol['Lime'] = - emission.imol['Lime'] * 1.2
