...
def create_HP_sys(ins, outs):
    ...
    M402 = bst.units.Mixer('M402', ins=(D401-1,
                                        'dilution_water2', 'recycled_hot_water'))
    ...
    def M402_objective_fn(Water_imol):
        if Water_imol>=0.:
            ...
            M402.ins[1].empty()
            tot_water_recycle = S405.ins[0].imol['Water']
            if -Water_imol>tot_water_recycle:
                Water_imol = -tot_water_recycle
                
            S405.outs[0].copy_like(S405.ins[0])
            S405.outs[1].imol['Water'] = -Water_imol
            
            S405.outs[0].imol['Water'] += Water_imol
    ...
    def M402_adjust_water():
        flx.IQ_interpolation(M402_objective_fn, -10000, 10000, maxiter=50, ytol=1e-2)
    ...
    D402_t_H = bst.units.HXutility('D402_H', ins=D402-0, V=0., rigorous=False)
    
    # recycling water makes system convergence fail
    
    S405 = bst.units.FakeSplitter('S405', ins=(D402_t_H-0,))
    S405-0-2-M402
    # D402_t_H-0-2-M402
    ...
    M501 = bst.units.Mixer('M501', ins=(F301_P-0, F401-1, S404-0, S405-1)) # without sugars recycle
