...
def create_HP_sys(ins, outs):
    ...
    all_chemicals = [*sc.chemicals, *HP_chemicals]
    HP_chemicals.AceticAcid._CAS, AceticAcidCAS = 'AceticAcid', HP_chemicals.AceticAcid.CAS
    HP_chemicals.DAP._CAS, DAPCAS = 'DAP', HP_chemicals.DAP.CAS
    chemicals = {i.ID: i for i in all_chemicals}
    chemicals = tmo.Chemicals(chemicals.values())
    HP_chemicals.AceticAcid._CAS = AceticAcidCAS
    HP_chemicals.DAP._CAS = DAPCAS
    bst.settings.set_thermo(chemicals)
    
    def fix_split(isplit, ID):
        isplit['Cellulose'] = isplit['Glucan']
        isplit['Hemicellulose'] = isplit['Xylan']
    ...
    M501 = bst.units.Mixer('M501', ins=(D402_t_H-0, F401-1, S404-0)) # without sugars recycle
    ...
    M505 = bst.units.Mixer('M505', ins=(S503-1, bagasse, S401-0), 
    ...
    from biosteam import main_flowsheet as f
    s = f.stream
    makeup_water_streams = (BT.ins[-1], CT.ins[-1])
    ...
    PWC = bst.ProcessWaterCenter('PWC',
                                 (system_makeup_water, makeup_water, F301_P-0),
                                 (),
                                 None,
                                 makeup_water_streams,
...
HP_tea = CellulosicEthanolTEA(system=HP_sys, IRR=0.10, duration=(2016, 2046),
        ...
        OSBL_units=(u.WWT_cost, u.T603, u.T606, u.T606_P,
