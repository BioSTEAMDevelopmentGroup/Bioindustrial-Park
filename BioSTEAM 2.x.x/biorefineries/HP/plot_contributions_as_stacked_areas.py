...
for group in process_groups:
    ...
    cu_group_contributions[group.name] = []
...
for parameter in parameters:
    ...
    mc_dict = get_material_cost_breakdown()
    for group in process_groups:
...
cu_group_contributions_df = pd.DataFrame(cu_group_contributions, index = parameters)
...
with pd.ExcelWriter(file_to_save+'.xlsx') as writer:
    hu_group_contributions_df.transpose().to_excel(writer, sheet_name='Heat utility')
    cu_group_contributions_df.transpose().to_excel(writer, sheet_name='Cooling utility')
    pu_group_contributions_df.transpose().to_excel(writer, sheet_name='Power utility')
    ic_group_contributions_df.transpose().to_excel(writer, sheet_name='Installed capital')
    mc_group_contributions_df.transpose().to_excel(writer, sheet_name='Material cost')
