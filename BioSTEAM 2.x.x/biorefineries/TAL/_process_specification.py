...
def evaluate_across_specs(spec, system,
    ...
    try:
        ...
        for i in range(3):
            system.simulate()
    except : # (ValueError, RuntimeError) (ValueError, AssertionError)
...
class ProcessSpecification(bst.process_tools.ReactorSpecification):
    ...
    def load_yield(self, yield_):
        ...
        reactor.xylose_to_TAL_rxn.X = self.xylose_utilization_fraction * yield_
        
        if (reactor.glucose_to_TAL_rxn.X + reactor.glucose_to_VitaminA_rxn.X \
            + reactor.glucose_to_VitaminD2_rxn.X+ reactor.glucose_to_microbe_rxn.X) > 0.999:
            
            reactor.glucose_to_VitaminA_rxn.X = (0.1/0.225) * (0.999 - reactor.glucose_to_TAL_rxn.X)
            reactor.glucose_to_VitaminD2_rxn.X = (0.1/0.225) * (0.999 - reactor.glucose_to_TAL_rxn.X)
            reactor.glucose_to_microbe_rxn.X = (0.025/0.225) * (0.999 - reactor.glucose_to_TAL_rxn.X)
            # print(reactor.glucose_to_VitaminA_rxn.X)
            # print(reactor.glucose_to_microbe_rxn.X)
        if (reactor.xylose_to_TAL_rxn.X + reactor.xylose_to_VitaminA_rxn.X \
           + reactor.xylose_to_VitaminD2_rxn.X  + reactor.xylose_to_microbe_rxn.X) > 0.999:
            
            reactor.xylose_to_VitaminA_rxn.X = (0.1/0.225) * (0.999 - reactor.xylose_to_TAL_rxn.X)
            reactor.xylose_to_VitaminD2_rxn.X = (0.1/0.225) * (0.999 - reactor.xylose_to_TAL_rxn.X)
            reactor.xylose_to_microbe_rxn.X = (0.025/0.225) * (0.999 - reactor.xylose_to_TAL_rxn.X)
            # print(reactor.glucose_to_VitaminA_rxn.X)
            # print(reactor.glucose_to_microbe_rxn.X)
            # reactor.xylose_to_TAL_rxn.X = 0
            # self.spec_2 = reactor.glucose_to_TAL_rxn.X = yield_
