...
def create_TAL_sys(ins, outs):
    ...
    S405 = bst.units.SolidsCentrifuge('S405', ins=M401-0, outs=('S405_solid_fraction', 'S405_liquid_fraction'),
                                # moisture_content=0.50,
                                moisture_ID='Ethanol',
                                split=find_split(S401_index,
                                                  S401_cell_mass_split,
    ...
    R401 = units.HydrogenationReactor('R401', ins = (S405-1, '', Hydrogen), outs = 'HMTHP',
                                      vessel_material='Stainless steel 316',)
    
    F402 = bst.Flash('F402', ins=R401-0, outs=('HMTHP', 'HMDHP_and_DHL'), V=1-1e-5, 
                     # P=101325/20.
                     P=101325.,
                     )
    
    R402 = units.DehydrationReactor('R402', ins = (F402-0, ''), outs = 'PSA',
                                               vessel_material='Stainless steel 316',
                                               tau = 12)
    
    F402-1-1-R401
    
    R403 = units.RingOpeningReactor('R403', ins = (R402-0, ''), outs = 'SA',
                                               vessel_material='Stainless steel 316',
                                               tau = 18)
    
    S403 = bst.FakeSplitter('S403', ins=R403-0, outs=('SA_and_PolyPSA', 'PSA'))
    
    @S403.add_specification
    def S403_spec():
        S403.outs[0].imol['SA', 'PolyPSA'] = S403.ins[0].imol['SA', 'PolyPSA']
        S403.outs[1].imol['PSA'] = S403.ins[0].imol['PSA']
    
    S403-1-1-R403
    
    S404 = bst.FakeSplitter('S404', ins=S403-0, outs=('SA', 'PolyPSA'))
    
    @S404.add_specification
    def S404_spec():
        S404.outs[0].imol['SA'] = S404.ins[0].imol['SA']
        S404.outs[1].imol['PolyPSA'] = S404.ins[0].imol['PolyPSA']
    ...
    ethanol_fresh = Stream('ethanol_fresh', 
                            price=price['Ethanol'],
                           # price = 0.,
                           )
    ...
    # 7-day storage time, similar to ethanol's in Humbird et al.
    T620 = units.TALStorageTank('T620', ins=S404-0, tau=7*24, V_wf=0.9,
...
def get_SA_MPSP():
    ...
    return SA.price*SA.F_mass/SA.imass['SA']
