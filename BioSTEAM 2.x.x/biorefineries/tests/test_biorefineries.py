...
def test_cornstover():
    ...
    assert np.allclose(product.price, 0.7181274736069763, rtol=1e-2)
    assert np.allclose(tea.sales, 130370882.0448293, rtol=1e-2)
    assert np.allclose(tea.material_cost, 81874586.24007271, rtol=1e-2)
    assert np.allclose(tea.installed_equipment_cost, 216746853.2961125, rtol=1e-2)
    assert np.allclose(tea.utility_cost, -12815777.722743921, rtol=1e-2)
    assert np.allclose(units.get_heating_duty(), 309.63747263325627, rtol=1e-2)
    assert np.allclose(units.get_cooling_duty(), 348.1567497678262, rtol=1e-2)
    assert np.allclose(units.get_electricity_consumption(), 21.892957260681392, rtol=1e-2)
    assert np.allclose(units.get_electricity_production(), 48.5353728440538, rtol=1e-2)
