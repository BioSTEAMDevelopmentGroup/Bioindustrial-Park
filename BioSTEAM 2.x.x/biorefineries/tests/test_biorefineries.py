...
def test_cornstover():
    ...
    assert np.allclose(product.price, 0.7361598918267038, rtol=1e-2)
    assert np.allclose(tea.sales, 135978837.909036, rtol=1e-2)
    assert np.allclose(tea.material_cost, 82447124.7444652, rtol=1e-2)
    assert np.allclose(tea.installed_equipment_cost, 215931558.08311233, rtol=1e-2)
    assert np.allclose(tea.utility_cost, -8901446.253698416, rtol=1e-2)
    assert np.allclose(units.get_heating_duty(), 310.1201936135629, rtol=1e-2)
    assert np.allclose(units.get_cooling_duty(), 381.93367039411356, rtol=1e-2)
    assert np.allclose(units.get_electricity_consumption(), 24.540694232460982, rtol=1e-2)
    assert np.allclose(units.get_electricity_production(), 46.20335309778644, rtol=1e-2)
    bst.process_tools.default()
