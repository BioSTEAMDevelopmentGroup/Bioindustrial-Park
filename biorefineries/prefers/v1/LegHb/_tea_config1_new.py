# -*- coding: utf-8 -*-
"""
TEA Configuration for LegHb system using _config1_new (Internalized Specifications).

Refactored from _tea_config1.py to work with the new config where:
...
Key changes vs _tea_config1.py:
    - Imports from _config1_new instead of _config1
...
from biorefineries.prefers.v1.LegHb.system._config1_new import seed_targets
...
class PreFerSTEA(PreFerSTEA_Base):
    ...
    # in _config1_new.py (empirical measurement per titer iteration).
    ...
    def set_production_rate(self, target_production_kg_hr, verbose=True):
        ...
        Delegates to _config1_new.set_production_rate which:
          - Scales all input streams proportionally
          - Relies on R302 spec for titer/NH3 convergence (no external loops)
        """
        from biorefineries.prefers.v1.LegHb.system._config1_new import set_production_rate
    ...
    def check_product_specifications(self):
        ...
        """
        from biorefineries.prefers.v1.LegHb.system._config1_new import check_LegHb_specifications
...
if __name__ == '__main__':
    ...
    import argparse
    import biosteam as bst
    from biorefineries.prefers.v1.LegHb.system._config1_new import create_LegHb_system
    ...
    parser = argparse.ArgumentParser(description='LegHemoglobin TEA Analysis (NEW config)')
    ...
    print("="*85)
    print(f"LEGHEMOGLOBIN TEA - CONFIG1_NEW (Internalized Specifications)")
    print("="*85)
    ...
    # 3. Create system using NEW config (R302 spec handles titer + NH3 internally)
    print(f"\n1. Creating LegHb system (config1_new - internalized specs)...")
