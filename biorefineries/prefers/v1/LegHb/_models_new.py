# -*- coding: utf-8 -*-
"""
Uncertainty & Sensitivity Model for LegHb system using _config1_new (Internalized Specifications).
...
from biorefineries.prefers.v1.LegHb.system._config1_new import create_LegHb_system, set_production_rate
...
def create_model(baseline_production_kg_hr=150, verbose=True):
    ...
    Uses _config1_new which has internalized titer/NH3 convergence in R302 specification.
    ...
    if verbose:
        print(f"Setting baseline production rate to {baseline_production_kg_hr} kg/hr (config1_new)...")
    ...
    # 1.1 Fermentation Titer Multiplier
...
def verify_model_integration():
    ...
    import traceback
    print("="*80)
    print("PREFERS MODEL INTEGRATION VERIFICATION (LegHb - config1_new)")
    print("="*80)
