# -*- coding: utf-8 -*-
"""
Baseline Data Generation for HemDx (NEW — Internalized Specifications)
=====================================================================

Identical to gen_data_base.py but imports from _models_new / _tea_config1_new
which use the internalized R302 add_specification approach.

Key differences vs gen_data_base.py:
    - Imports _models_new.create_model (supports config1/2/3 via _new modules)
    - Imports _tea_config1_new.PreFerSTEA
    - Much faster due to internalized spec (no external convergence loops)
"""
...
from biorefineries.prefers.v1.HemDx._models_new import create_model
from biorefineries.prefers.v1.HemDx._tea_config1_new import PreFerSTEA
...
def generate_baseline(config='config1', baseline_production_kg_hr=150, timestamp=None, results_dir=None):
    ...
    print("=" * 70)
    print("HEMDX - BASELINE METRICS (NEW — Internalized Specs)")
    print("=" * 70)
    print(f"\nConfiguration:")
    ...
    try:
        ...
        baseline_metrics_values = model([p.baseline for p in model.parameters])
        ...
        print(f"  [+] Saved sensitivity data: {sens_file}")
