# -*- coding: utf-8 -*-
"""
TEA Configuration for HemDx system using _config1 (Internalized Specifications).
...
Key changes vs _tea_config1.py:
    - Imports from _config1 instead of _config1
...
from biorefineries.prefers.v2.HemDx.system._config1 import seed_targets
...
class PreFerSTEA(PreFerSTEA_Base):
    ...
    # in _config1.py (empirical measurement per titer iteration).
    ...
    def set_production_rate(self, target_production_kg_hr, verbose=True):
        ...
        Delegates to _config1.set_production_rate which:
          - Scales all input streams proportionally
          - Relies on R302 spec for titer/NH3 convergence (no external loops)
        """
        from biorefineries.prefers.v2.HemDx.system._config1 import set_production_rate
    ...
    def check_product_specifications(self):
        ...
        """
        from biorefineries.prefers.v2.HemDx.system._config1 import check_HemDx_specifications
...
if __name__ == '__main__':
    import argparse
    from biorefineries.prefers.v2.HemDx.system._config1 import create_NHemDx_system
    ...
    parser = argparse.ArgumentParser(description='HemDx TEA Analysis (NEW)')
    ...
    print("="*85)
    print(f"HEMDX TEA (NEW) - WITH INTERNALIZED DESIGN SPECIFICATION")
    print("="*85)
    ...
    print(f"\n{'='*85}")
    print("TEA ANALYSIS COMPLETE (NEW)")
    print(f"{'='*85}")
