# -*- coding: utf-8 -*-
"""
TEA Configuration for HemDx system using _config3 (Internalized Specifications).

Refactored from _tea_config3.py to work with the new config where:
...
Config 3: Extracellular production (SF=0.9).

Key changes vs _tea_config3.py:
    - Imports from _config3 instead of _config3
    - optimize_NH3_loading method REMOVED (handled by R302 spec)
    - set_production_rate simplified (no adjust_glucose_for_titer call)
    - check_product_specifications imports from _config3
...
from biorefineries.prefers.v2.HemDx.system._config3 import seed_targets
...
class PreFerSTEA(PreFerSTEA_Base):
    ...
    # in _config3.py (empirical measurement per titer iteration).
    ...
    def set_production_rate(self, target_production_kg_hr, verbose=True):
        ...
        Delegates to _config3.set_production_rate which:
          - Scales all input streams proportionally
          - Relies on R302 spec for titer/NH3 convergence (no external loops)
        """
        from biorefineries.prefers.v2.HemDx.system._config3 import set_production_rate
    ...
    def check_product_specifications(self):
        ...
        """
        from biorefineries.prefers.v2.HemDx.system._config3 import check_HemDx_specifications
...
if __name__ == '__main__':
    import argparse
    from biorefineries.prefers.v2.HemDx.system._config3 import create_NHemDx_system
    from biorefineries.prefers.v2.HemDx._chemicals import create_chemicals_Hemodextrin
    from biorefineries.prefers.v2._process_settings import load_process_settings
    
    parser = argparse.ArgumentParser(description='HemDx TEA Analysis (Config 3 NEW)')
    ...
    print("="*85)
    print(f"HEMDX TEA (Config 3 NEW) - WITH INTERNALIZED DESIGN SPECIFICATION")
    print("="*85)
    ...
    print(f"\n1. Creating HemDx system (Config 3 NEW)...")
    ...
    print(f"\n{'='*85}")
    print("TEA ANALYSIS COMPLETE (Config 3 NEW)")
    print(f"{'='*85}")
