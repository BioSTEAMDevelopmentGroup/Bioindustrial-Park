...
from biorefineries.isobutanol.units import compute_isobutanol_titer, compute_isobutanol_mass
...
class ProcessSpecification(bst.process_tools.ReactorSpecification):
    ...
    def get_sugar_to_isobutanol_conversion(self, unit):
        return unit.glucose_to_isobutanol_rxn.X
    
    def get_sugar_conversion(self, unit):
        return self.get_sugar_to_isobutanol_conversion(unit) +\
    ...
    def load_yield(self, yield_):
        ...
        get_sugar_to_isobutanol_conversion = self.get_sugar_to_isobutanol_conversion
        
        self.spec_1 = reactor.glucose_to_isobutanol_rxn.X = reactor.acetate_to_isobutanol_rxn.X = yield_
        seed_train.glucose_to_isobutanol_rxn.X = seed_train.acetate_to_isobutanol_rxn.X = yield_ * seed_train.ferm_ratio
        ...
        for unit in [reactor, seed_train]:
            ...
            unit.xylose_to_isobutanol_rxn.X = yield_ * unit.ferm_ratio
            unit.acetate_to_isobutanol_rxn.X = yield_ * unit.ferm_ratio
            sugar_to_isobutanol_conversion = get_sugar_to_isobutanol_conversion(unit)
            ...
            if self.byproduct_yields_decrease_policy == 'sequential, when product yield too high':
                if sugar_to_isobutanol_conversion>=\
                    1. - regular_microbe_conversion - regular_citric_acid_conversion:
                    # first decrease citric acid production if needed
                    unit.glucose_to_CitricAcid_rxn.X =\
                    unit.xylose_to_CitricAcid_rxn.X =\
                        max(1e-6, 1.-1e-6 - sugar_to_isobutanol_conversion - regular_microbe_conversion)
                    ...
                    # then decrease cell mass yield if needed
                        unit.glucose_to_microbe_rxn.X =\
                        unit.xylose_to_microbe_rxn.X =\
                            max(1e-6, 1.-2e-6 - sugar_to_isobutanol_conversion)
                ...
                c1, c2 = -m1, -m2
                unit.glucose_to_CitricAcid_rxn.X = max(1e-6, m1*sugar_to_isobutanol_conversion + c1)
                unit.xylose_to_CitricAcid_rxn.X = max(1e-6, m1*sugar_to_isobutanol_conversion + c1)
                unit.glucose_to_microbe_rxn.X = max(1e-6, m2*sugar_to_isobutanol_conversion + c2)
                unit.xylose_to_microbe_rxn.X = max(1e-6, m2*sugar_to_isobutanol_conversion + c2)
    ...
    def load_dehydration_conversion(self, conversion):
        ...
        self.spec_1 = dr.isobutanol_to_MEK_rxn.X = conversion
        dr.isobutanol_to_IBA_rxn.X = 0.5 * conversion # original conversions are 0.52 and 0.26, maintain ratio
        if dr.isobutanol_to_MEK_rxn.X + dr.isobutanol_to_IBA_rxn.X > 0.999:
            dr.isobutanol_to_IBA_rxn.X = 0.999 - conversion
...
class TiterAndInhibitorsSpecification:
    ...
    def __init__(self, evaporator, pump, mixer, heat_exchanger, seed_train_system, reactor, 
        ...
        self.get_products_mass = compute_isobutanol_mass
        self.seed_train_system = seed_train_system
    ...
    def calculate_titer(self):
        ...
        # return compute_isobutanol_titer(self.product)
