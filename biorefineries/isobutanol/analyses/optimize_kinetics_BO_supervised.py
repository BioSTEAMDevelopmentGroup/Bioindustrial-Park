...
sequentially) -- ask-first, like the unsupervised driver. Examples:
    ...
    # inhibition effector (all families on the default 0.75x-1.5x band), spike pinned
    # (name ..._ib0.75-1.5_xk10+k7+k8_...):
...
def default_study_name(scenario, objective, kinetic_bounds_scenario,
    ...
    which puts every family on the default band (0.75x-1.5x), so the tag
    collapses to `_ib0.75-1.5`. `group_multiplier_bounds` (None = that
    ...
    if study_target_products is not None:
        ...
        # (metabolic_minimal tags its group band _ib0.75-1.5 -- every family
...
if __name__ == '__main__':
    ...
    parser.add_argument('--study-type', default=ko.DEFAULT_STUDY_TYPE,
                        ...
                        help='study preset axis 2: metabolic = capacity, '
                             ...
                             '(every family on the default 0.75x-1.5x band), '
                             'no K_* '
                             'terms, spike feed pinned at the baseline '
                             '(24 variables; name tags _ib0.75-1.5_xk10+k7+k8); '
                             ...
                             'variables; 10 for ethanol_only; tags _ib0.75-1.5 '
                             'only)')
