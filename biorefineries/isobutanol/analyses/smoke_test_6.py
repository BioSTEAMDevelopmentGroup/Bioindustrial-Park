...
- purity-adjusted ethanol MPSP within 1% of 1.6693 (was 1.6625 before the
  ...
  and recorded in CLAUDE.md; well above the both-trains B baseline 0.60042
...
def load_simulate_baseline(stream_IDs=('ethanol', 'isobutanol'),
    ...
    # Ethanol-only build: config-specific pins (NOT the registry B pins).
    # B's IBO goes unrecovered to WWT in this build, so isobutanol is nan.
