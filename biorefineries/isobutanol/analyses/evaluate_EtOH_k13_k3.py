...
def get_IRR():
    # IRR at default product prices; side-effect free (see solve_TEA)
    return solve_TEA(stream_IDs=())['IRR']
    
metrics = {
            # 'MPSP': {'f': get_product_MPSP, 'units': '$/kg'},
            'IRR': {'f': get_IRR, 'units': '%'},
...
for s3 in spec_3:
    ...
    for s2 in spec_2:
        ...
        for s1 in spec_1:
            ...
            try:
                if s1==s2==0.0:
                    raise ValueError('Infeasible combination (both parameters are zero).')
