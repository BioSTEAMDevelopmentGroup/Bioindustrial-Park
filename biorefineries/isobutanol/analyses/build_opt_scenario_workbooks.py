...
# for license details.
"""
One-shot generator (ASK-FIRST to run; it simulates) for the four remaining
metabolic_minimal_subset opt_* kinetic-optimum scenario workbooks.

NOTE (2026-09-20): opt_IRR was RELOCATED to trial 1602 of the
metabolic_split_12d PI (log-tail) GP study and is now generated by its own
`analyses/build_opt_IRR_split12d_workbook.py` (ask-first). It has therefore
been REMOVED from the STUDIES list below, so re-running this generator will
NOT overwrite the relocated opt_IRR workbook. (See the spec
docs/superpowers/specs/2026-09-20-opt-IRR-relocate-to-split12d-optimum-design.md.)
...
For each of the four remaining metabolic_minimal_subset studies (IBO titer,
IBO yield, EtOH titer, EtOH yield), it:
...
# opt_IRR is intentionally ABSENT: as of 2026-09-20 it is sourced from
# metabolic_split_12d trial 1602, not this metabolic_minimal_subset IRR study's
# trial 774, and is generated by build_opt_IRR_split12d_workbook.py. Re-adding
# the ('opt_IRR', 'IRR', _STEM + 'irr' + _TAIL, 774) entry here would OVERWRITE
# the relocated workbook -- do not.
STUDIES = [
