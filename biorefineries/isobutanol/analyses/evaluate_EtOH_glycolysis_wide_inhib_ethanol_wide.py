...
This is the BOTH-AXES-WIDE variant: BOTH grouped multipliers are widened well
past the study's default bands. The x-axis (glycolysis multiplier) spans
**1e-3x-4x** the scenario-A baseline instead of the study's 0.2x-4x (reaching
down to effective glycolysis knock-out), and the y-axis (inhib_ethanol
multiplier) spans **1e-3x-2x** instead of 0.75x-1.5x (reaching down to
effectively no ethanol inhibition). Output filenames carry both an
``_rb{lo}-{hi}`` glycolysis-band tag and an ``_ib{lo}-{hi}`` inhib-band tag, so
this run's CSVs / plots coexist with the 0.2x-4x-glycolysis siblings' under
analyses/results/.
...
is unchanged at 4x, which the 2026-09-20 sibling runs found fully feasible in
...
wide-band sibling (glycolysis 0.2x-4x) had 5 such NaN cells, all in the
...
# 0.2x-4x-glycolysis siblings' (all share the same x/y/z labels).
