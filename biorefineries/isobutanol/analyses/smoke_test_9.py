...
# for license details.
"""
Smoke test 9 -- scenario opt_IRR (trial #1602 of the 2026-09-16
scenario-A-anchored metabolic_split_12d PI (log-tail) GP study -- the
highest-IRR point of the campaign, kept as the IRR-objective pin; until
2026-09-20 this was best-IRR trial #774 of the 2026-09-07
metabolic_minimal_subset IRR study) with ONLY the IBO/EtOH separation
train: ``isobutanol.load(separation_processes=('IBO_EtOH',))``.

Reproduces that trial: the workbook's Baseline column carries the trial's
exact simulated kinetic state (scenario-A held/non-sampled params + the
sampled rates k_3 / k_6 / k_13 / k_17 + the expanded glycolysis /
ehrlich_downstream / inhibition group members + INTENDED (non-derated)
k_7/k_8, which the A-referenced enzyme burden derates at the
load_simulate choke point; generated by
analyses/build_opt_IRR_split12d_workbook.py), and the registry carries
the trial's feeding strategy and reproduced pins. IBO_EtOH-only ==
both-trains at S201 split 1.0 (the studies' build), so the pins transfer
exactly.
