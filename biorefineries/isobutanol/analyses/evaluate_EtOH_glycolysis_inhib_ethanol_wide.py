...
# for license details.
"""
2-D kinetic sweep: the grouped ``glycolysis`` CAPACITY family MULTIPLIER
(x-axis) vs the grouped ``inhib_ethanol`` family MULTIPLIER (y-axis), on the
scenario-A baseline, WITH the enzyme burden turned ON.

This is the WIDE-inhib-band variant of ``evaluate_EtOH_glycolysis_inhib_
ethanol.py``: the y-axis (inhib_ethanol multiplier) spans **1e-3x-2x** the
scenario-A baseline instead of the study's default 0.75x-1.5x, reaching down to
effectively NO ethanol inhibition. The x-axis (glycolysis multiplier) is
unchanged. Output filenames carry an ``_ib{lo}-{hi}`` tag so both runs' CSVs /
plots coexist under analyses/results/.
...
``evaluate_*`` scripts). ``(glycolysis multiplier = 1.0, inhib_ethanol
multiplier = 1.0)`` lies on the grid interior and reproduces the scenario-A
baseline point.
...
Burden feasibility is set by the x-axis alone: the ``inhib_ethanol``
coefficients are not proteome pools, so widening THEIR band does not change
which points are burden-feasible. The glycolysis x-band is unchanged (0.2x-4x),
and the narrow-band sibling run (2026-09-20) found that band fully feasible in
scenario A (Phi_M ~0.21 < ``F_flex`` = 0.245 even at 4x), so this wide-band run
is likewise expected to have 0 infeasible cells -- the burden only derates
growth (``k_7``/``k_8``) at every point. The wider inhib band instead probes
the fermentation response: at 1e-3x (no ethanol inhibition) the ethanol titer
should rise well above the baseline.
"""
...
# Snapshot each grouped family's scenario-A baseline off the live model
# (member lists sourced from kinetic_optimization -- no hardcoding; a member
# absent from the model is dropped, mirroring expand_grouped_values).
_available = ko.discover_kinetic_parameters(r)

GLYCOLYSIS_MEMBERS = [m for m in ko.METABOLIC_14D_RATE_GROUPS['glycolysis']
                      if m in _available]
baseline_glycolysis = {m: _available[m] for m in GLYCOLYSIS_MEMBERS}
...
print('\nScenario-A glycolysis baselines: %s' % baseline_glycolysis)
print('Scenario-A inhib_ethanol baselines: %s' % baseline_inhib_ethanol)


def apply_glycolysis_multiplier(multiplier):
    """Set every glycolysis member to its scenario-A baseline x `multiplier`
    (intra-family ratios preserved; == expand_grouped_values for the single
    glycolysis group)."""
    for member, base in baseline_glycolysis.items():
...
# `_ib{lo}-{hi}` tags the (wide) inhib_ethanol band so this run's CSVs / plots
# do not collide with the default-band sibling's (same x/y/z labels).
_ib_tag = f'_ib{INHIB_ETHANOL_MULTIPLIER_BOUNDS[0]:g}-{INHIB_ETHANOL_MULTIPLIER_BOUNDS[1]:g}'
file_to_save = f'ibo_{steps}_{x_label[:5]}_{y_label[:5]}_{z_label[:5]}{_ib_tag}_opt={perform_feeding_strategy_opt}_max_n={ferm_reactor.nsk_kinetic_model.default_max_n_glu_spikes}_'
    ...
    for s2 in spec_2:
        ...
        for s1 in spec_1:
            ...
            try:
                ...
                apply_glycolysis_multiplier(s1)
                ...
                error_message = str_e
                # A burden-infeasible point (high glycolysis multiplier over the
                # proteome cap) is an EXPECTED NaN, not a solver failure -- keep
                # it out of errors_dict so the console log flags only genuine
                # issues.
