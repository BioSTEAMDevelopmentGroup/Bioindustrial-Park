...
def load_spec_3(val):
    rxn_sys.reactions[2].reaction.rate_params['E_per_C'] = val * MW_C5H7O2N_by_5 / MW_Yeast
    
def load_spec_1(val):
    rxn_sys.reactions[2].reaction.rate_params['kcat'] = val

def load_spec_2(val):
    rxn_sys.reactions[2].reaction.rate_params['KM'] = val
    

steps = (1, 30, 30)
# spec_3_vector = np.linspace(494_000/2, 494_000*2, steps[0])
spec_3_vector = np.array([494_000/5])
spec_1_vector = np.linspace(50, 500, steps[1])
...
z_label = r"$\bfMitochondrial ADH$" # title of the z axis
...
if plot: 
    ...
    MPSP_w_levels = np.arange(0.5, 1.01, 0.01)
    MPSP_cbar_ticks = np.arange(0.5, 1.01, 0.05)
    MPSP_w_ticks = []
