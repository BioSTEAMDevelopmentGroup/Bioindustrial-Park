...
for p in E_per_Cs:
    ...
    for y in umaxes:
        ...
        for t in k_inhibs_ethanol:
            ...
            try:
                ...
                titers_mol_per_mol_total[-1].append(broth.imol['Isobutanol']/broth.imol['Isobutanol', 'Water'].sum())
    ...
    pd.DataFrame(d1_Metric1).to_csv(isobutanol_results_filepath+'MPSP-'+csv_file_to_save+'.csv')
    pd.DataFrame(d1_Metric2).to_csv(isobutanol_results_filepath+'GWP-'+csv_file_to_save+'.csv')
    pd.DataFrame(d1_Metric3).to_csv(isobutanol_results_filepath+'FEC-'+csv_file_to_save+'.csv')
    pd.DataFrame(d1_Metric4).to_csv(isobutanol_results_filepath+'AOC-'+csv_file_to_save+'.csv')
    pd.DataFrame(d1_Metric5).to_csv(isobutanol_results_filepath+'TCI-'+csv_file_to_save+'.csv')
    pd.DataFrame(d1_Metric6).to_csv(isobutanol_results_filepath+'Purity-'+csv_file_to_save+'.csv')
...
np.save(isobutanol_results_filepath+'MPSP-'+file_to_save, results_metric_1)
np.save(isobutanol_results_filepath+'GWP-'+file_to_save, results_metric_2)
np.save(isobutanol_results_filepath+'FEC-'+file_to_save, results_metric_3)
...
if plot: 
    ...
    contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_1, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                    ...
                                    # x_data = yields/theoretical_max_g_isobutanol_acid_per_g_glucose,
