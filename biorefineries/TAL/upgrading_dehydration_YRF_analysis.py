...
def load_dehydration_spent_catalyst_replacement_rate(dehydration_cat_replacement_rate):
    R402.spent_catalyst_replacements_per_year = dehydration_cat_replacement_rate
    
def load_dehydration_mcat_frac(dehydration_mcat_frac):
    try: R402.mcat_frac = dehydration_mcat_frac[0]
    except: R402.mcat_frac = dehydration_mcat_frac

spec_upgrading.load_spec_1, spec_upgrading.load_spec_2, spec_upgrading.load_spec_3 = \
    load_dehydration_PSA_yield, load_dehydration_spent_catalyst_replacement_rate, load_dehydration_mcat_frac
...
spec_2 = dehydration_cat_replacement_rates = np.linspace(0.001, 5., steps[1]) # HMTHP->PSA conversion in the dehydration reactor
spec_3 = dehydration_mcat_fracs = np.linspace(0.01, 0.999, steps[2])# PSA->SA conversion in the ring opening & hydrolysis reactor
...
y_label = r"$\bfDehydration$" + " " + r"$\bfCatalyst$" + " " + r"$\bfReplacement$" + " " + r"$\bfRate$" # title of the y axis
y_units = r"$\mathrm{y}^{-1}$"
y_ticks=np.arange(0., 5.5, 1.)


z_label = r"$\bfDehydration$" + " " + r"$\bfCatalyst:Reactant$" + " " + r"$\bfRatio$" # title of the z axis
z_units =  r"$\mathrm{g}\cdot\mathrm{g}^{-1}$"
...
# %% Run TRY analysis 
for p in dehydration_mcat_fracs:
...
contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_1, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                ...
                                y_data=dehydration_cat_replacement_rates, # y axis values
                                z_data=dehydration_mcat_fracs, # z axis values
                                ...
                                w_ticks=[0, 8.0, 10.0, 15.0, 20.0], # labeled, lined contours; a subset of w_levels
                                x_units=x_units,
                                y_units=y_units,
                                z_units=z_units,
                                w_units=MPSP_units,
