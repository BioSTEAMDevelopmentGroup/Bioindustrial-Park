...
def load_ringopeninghydrolysis_SA_yield(SA_yield):
    R403.PSA_to_SA_rxn.X = SA_yield

spec_upgrading.load_spec_1, spec_upgrading.load_spec_2, spec_upgrading.load_spec_3 = \
    load_hydrogenation_HMTHP_yield, load_dehydration_PSA_yield, load_ringopeninghydrolysis_SA_yield
...
spec_1 = HMTHP_yields = np.linspace(0.3, 0.999, steps[0]) # TAL->HMTHP conversion in the hydrogenation reactor
spec_2 = PSA_yields = np.linspace(0.3, 0.999, steps[1]) # HMTHP->PSA conversion in the dehydration reactor
spec_3 = SA_yields = np.linspace(0.3, 0.999, steps[2])# PSA->SA conversion in the ring opening & hydrolysis reactor
...
x_label = r"$\bfHydrogenation$" + " " + r"$\bfHMTHP$" + " " + r"$\bfYield$" # title of the x axis
x_units = r"$\mathrm{\%}$" + " " + r"$\mathrm{theoretical}$"
x_ticks=np.arange(0.3, 1.1, 0.1)

y_label = r"$\bfDehydration$" + " " + r"$\bfPSA$" + " " + r"$\bfYield$" # title of the y axis
y_units = r"$\mathrm{\%}$" + " " + r"$\mathrm{theoretical}$"
y_ticks=np.arange(0.3, 1.1, 0.1)

z_label = r"$\bfRing$" + "-" + r"$\bfOpening$" + " " + r"$\bfSA$" + " " + r"$\bfYield$" # title of the z axis
z_units = r"$\mathrm{\%}$" + " " + r"$\mathrm{theoretical}$"
z_ticks=np.arange(0.3, 1.1, 0.1)
...
contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_1, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                x_data=HMTHP_yields, # x axis values
                                # x_data = yields/theoretical_max_g_TAL_per_g_glucose,
                                y_data=PSA_yields, # y axis values
                                z_data=SA_yields, # z axis values
