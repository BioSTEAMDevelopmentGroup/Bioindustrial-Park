...
yield_paramindex, titer_paramindex, productivity_paramindex = (17, 18, 19)


#%% Utils

class UncertaintyBasedMetricEvaluator():
    def __init__(self, ID, model,
                 metrics = [],
                 N_simulations_per_evaluation=50,
                 sampling_rule='L',
                 exception_hook='warn',
                 notification_interval=25,
                 autoload=None, autosave=None, file=None):
        self.ID = ID
        self._model = model
        self.metrics = []
        self.N_simulations_per_evaluation = N_simulations_per_evaluation
        self.sampling_rule = sampling_rule
        self.exception_hook = exception_hook
        self.notification_interval = notification_interval
        self.autoload, self.autosave, self.file = autoload, autosave, file
    ...
    def update_parameter_baseline(self, index, baseline):
        self._model.parameters[index].baseline = baseline
    def update_parameter_distribution(self, index, distribution):
        self._model.parameters[index].distribution = distribution
    
    def load_samples(self):
        samples = model.sample(N=self.N_simulations_per_evaluation, rule=self.sampling_rule)
        model.load_samples(samples)
        model.exception_hook = self.exception_hook
        
    def evaluate(self):
        self.load_samples()
        self._model.evaluate(notify=self.notification_interval, 
                             autoload=self.autoload, autosave=self.autosave, file=self.file)
        # return [i() for i in self.metrics]


class UncertaintyBasedMetric():
    def __init__(self, ID, model, model_metric_index, comparison_distribution):
        self.ID = ID
        self._model = model
        self.model_metric_index = model_metric_index
    ...
    @property
    def model_metric_index_in_eval_table(self):
        return len(self._model.parameters) + self.model_metric_index
    
    @property
    def model_metric_distribution(self):
        model = self._model
        return np.array(model.table[model.table.keys()[self.model_metric_index_in_eval_table]])
    
    def get_value(self):
        model_metric_distribution = self.model_metric_distribution
        comparison_distribution = self.comparison_distribution
        return np.sum([np.where(model_metric_distribution<i)[0].shape[0] for i in comparison_distribution]) \
            / (model_metric_distribution.shape[0] * comparison_distribution.shape[0])

def get_new_yield_titer_prod_distributions(
                          new_baseline_values=[spec.baseline_yield,
                                               spec.baseline_titer,
                                               spec.baseline_productivity], 
                          upper_bound_relative=1.2, 
                          lower_bound_relative=0.8, 
                          distribution_type='Uniform',
                          min_values=(0.04, 4., 0.01),
                          max_values=(0.96, 101., np.inf),
                          ):
    if distribution_type=='Uniform':
        return [distributions.Uniform(
                              min(max_values[i], max(min_values[i], lower_bound_relative*new_baseline_values[i])), 
                              min(max_values[i], max(min_values[i], upper_bound_relative*new_baseline_values[i])))
                for i in range(len(new_baseline_values))]
    elif distribution_type=='Triangular':
        return [distributions.Triangle(
                              min(max_values[i], max(min_values[i], lower_bound_relative*new_baseline_values[i])), 
                              new_baseline_values[i],
                              min(max_values[i], max(min_values[i], upper_bound_relative*new_baseline_values[i])))
                for i in range(len(new_baseline_values))]
    
    return ValueError(f"distribution_type must be 'Uniform' or 'Triangular', not {distribution_type}")

#%%  Define PMC-like metrics

market_distribution = np.linspace(SA_market_range[0], SA_market_range[1], 100)
GWP_baseline_distribution = np.linspace(TAL_lca.GWP, TAL_lca.GWP, 100)
FEC_zero_distribution = np.linspace(0, 0, 100)

PMClikeEvaluator = UncertaintyBasedMetricEvaluator('PMClikeEvaluator', model=model)

PMC = UncertaintyBasedMetric('PMC', model=model, 
                             model_metric_index=3, 
                             comparison_distribution=market_distribution)

P_GWP_less_than_baseline = UncertaintyBasedMetric('P_GWP_less_than_baseline', model=model, 
                             model_metric_index=246-31, 
                             comparison_distribution=GWP_baseline_distribution)

P_FEC_negative = UncertaintyBasedMetric('P_FEC_negative', model=model, 
                             model_metric_index=261-31, 
                             comparison_distribution=FEC_zero_distribution)

#%%
# simulate_and_print()

baseline_yield_dist, baseline_titer_dist, baseline_prod_dist = \
    model.parameters[yield_paramindex].distribution, \
    model.parameters[titer_paramindex].distribution, \
    model.parameters[productivity_paramindex].distribution, \
    
def load_simulate_baseline():
    PMClikeEvaluator.update_parameter_baseline(yield_paramindex, spec.baseline_yield)
    PMClikeEvaluator.update_parameter_baseline(titer_paramindex, spec.baseline_titer)
    PMClikeEvaluator.update_parameter_baseline(productivity_paramindex, spec.baseline_productivity)
    
    PMClikeEvaluator.update_parameter_distribution(yield_paramindex, baseline_yield_dist)
    PMClikeEvaluator.update_parameter_distribution(titer_paramindex, baseline_titer_dist)
    PMClikeEvaluator.update_parameter_distribution(productivity_paramindex, baseline_prod_dist)
...
TAL_metrics = [get_product_MPSP, 
               ...
               # get_product_purity,
               PMC.get_value,
               P_GWP_less_than_baseline.get_value,
               P_FEC_negative.get_value,
               ]
...
for p in productivities:
    ...
    for y in yields:
        ...
        for t in titers:
            ...
            error_message = None
            try:
                load_simulate_baseline()
                simulate_and_print()
                
                spec.load_specifications(spec_1=y, spec_2=t, spec_3=p)
                
                PMClikeEvaluator.update_parameter_baseline(yield_paramindex, y)
                PMClikeEvaluator.update_parameter_baseline(titer_paramindex, t)
                PMClikeEvaluator.update_parameter_baseline(productivity_paramindex, p)
                
                new_distributions=get_new_yield_titer_prod_distributions(new_baseline_values=(y,t,p))
                PMClikeEvaluator.update_parameter_distribution(yield_paramindex, new_distributions[0])
                PMClikeEvaluator.update_parameter_distribution(titer_paramindex, new_distributions[1])
                PMClikeEvaluator.update_parameter_distribution(productivity_paramindex, new_distributions[2])
...
MPSP_w_ticks = [3.5, 3.75, 4, 4.5, 5, 10]
...
# Purity_w_levels, Purity_w_ticks, Purity_cbar_ticks = get_contour_info_from_metric_data(results_metric_4,)
Purity_w_levels = np.arange(0.1, 1.0, 0.01)
Purity_cbar_ticks = np.arange(0.1, 1.0, 0.05)
# Purity_cbar_ticks = np.arange(2, 8.1, 1.)
Purity_w_ticks = [.85, 0.88, .90, .91, 0.92, 0.93, 0.94, 0.95]
# Purity_w_levels = np.arange(0., 15.5, 0.5)

contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_4, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., Purity
                                x_data=100*yields, # x axis values
                                # x_data = yields/theoretical_max_g_TAL_acid_per_g_glucose,
                                y_data=titers, # y axis values
                                z_data=productivities, # z axis values
