...
def solve_addition_for_mol_ratio_and_decarb_conv(mol_PD_presence_per_mol_TAL, x):
    U402.decarboxylation_conversion = x
    # def obj_f(addn_per_mol_TAL_in):
    ...
    # return IQ_interpolation(obj_f, 0., 3.5, ytol=5e-3)
    
    # U402.simulate()
    # mol_TAL = U402.outs[0].imol['TAL']
    # mol_acetylacetone_from_decarb = mol_TAL/(1.-x)
    M401.mol_acetylacetone_per_mol_TAL = y = mol_PD_presence_per_mol_TAL - (1./(1.-x)) + 1.
    
    if y<0: raise ValueError(f'Required acetylacetone:TAL presence ratio {mol_PD_presence_per_mol_TAL} too low to achieve with given decarboxylation conversion {x*100}%.')
...
def load_M401_acetylacetone_req(mol_acetylacetone_per_mol_TAL):
    ...
    solve_addition_for_mol_ratio_and_decarb_conv(mol_acetylacetone_per_mol_TAL, U402.decarboxylation_conversion)
...
M401_addition_w_label = r"$\bfAcetylacetone$"  +" "+ r"$\bfAddition$" +"\n"+ r"$\bfAdded$" # title of the color axis
...
MPSP_w_ticks = [3.25, 3.5, 3.75, 4, 4.5, 5, 9, 10]
...
GWP_w_ticks = [2, 3, 4, 6, 8, 10, 14, 16, 20]
...
AOC_w_ticks = np.arange(0, 120.1, 20)
...
# Purity_w_ticks = np.arange(0.78, 0.921, 0.02)
Purity_w_ticks = [87, 88, 89, 90, 91]
...
M401_addition_w_levels = np.arange(0., 5.1, 0.25)
M401_addition_cbar_ticks = np.arange(0., 5.1, 0.5)
M401_addition_w_ticks = [0, 0.25] + list(np.arange(0.5, 5.1, 0.5))
