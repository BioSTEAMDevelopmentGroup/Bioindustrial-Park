...
U402, M401, F403 = u.U402, u.M401, u.F403
...
def load_F403_recycle_split(V):
    # M401.mol_acetylacetone_per_mol_TAL = mol_acetylacetone_per_mol_TAL
    # solve_V_for_mol_ratio_and_decarb_conv(mol_acetylacetone_per_mol_TAL, U402.decarboxylation_conversion)
    F403.V = V
...
get_F403_V = lambda: F403.V

TAL_metrics = [get_product_MPSP, lambda: TAL_lca.GWP, lambda: TAL_lca.FEC, 
               get_TAL_AOC, get_TAL_FCI, get_product_purity, get_F403_V]
...
spec_2 = F403_recycle_Vs = np.linspace(0.01, 0.99, steps[1]) # titer
...
y_label = r"$\bfSupernatant$" +" "+ r"$\bfVapor$" +" "+ r"$\bfRecycled$" # title of the y axis
y_units = r"$\mathrm{\%}$"
...
F403_V_w_label = r"$\bfSupernatant$"  +" "+ r"$\bfRecycling$" +" "+ r"$\bfRequired$" # title of the color axis
F403_V_units = r"$\mathrm{\%}$"
...
# spec.load_specifications(TAL_decarb_convs[0], F403_recycle_Vs[0], PD_prices[0])
...
curr_no = 0
total_no = len(TAL_decarb_convs)*len(F403_recycle_Vs)*len(PD_prices)
...
for p in PD_prices:
    ...
    for y in TAL_decarb_convs:
        ...
        for t in F403_recycle_Vs:
            ...
            except FloatingPointError as e1:
                d1_Metric1[-1].append(np.nan)
                d1_Metric2[-1].append(np.nan)
                d1_Metric3[-1].append(np.nan)
                d1_Metric4[-1].append(np.nan)
    ...
    pd.DataFrame(d1_Metric7).to_csv(TAL_results_filepath+'F403_split-'+file_to_save+'.csv')
...
contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_1, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                ...
                                y_data=100*F403_recycle_Vs, # y axis values
...
# F403_V_w_levels, F403_V_w_ticks, F403_V_cbar_ticks = get_contour_info_from_metric_data(results_metric_6,)
F403_V_w_levels = F403_V_cbar_ticks = F403_V_w_ticks = np.arange(0., 1.0, 0.05)
# F403_V_cbar_ticks = np.arange(2, 8.1, 1.)
# F403_V_w_ticks = [ 4, 4.5, 5, 8]
# F403_V_w_levels = np.arange(0., 15.5, 0.5)

contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_7, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., F403_split
                                x_data=100*TAL_decarb_convs, # x axis values
                                # x_data = TAL_decarb_convs/theoretical_max_g_TAL_acid_per_g_glucose,
                                y_data=100*F403_recycle_Vs, # y axis values
                                z_data=PD_prices, # z axis values
                                ...
                                y_label=y_label, # title of the y axis
                                z_label=z_label, # title of the z axis
                                w_label=F403_V_w_label, # title of the color axis
                                x_ticks=100*x_ticks,
                                y_ticks=y_ticks,
                                z_ticks=z_ticks,
                                w_levels=F403_V_w_levels, # levels for unlabeled, filled contour areas (labeled and ticked only on color bar)
                                w_ticks=F403_V_w_ticks, # labeled, lined contours; a subset of w_levels
                                x_units=x_units,
                                y_units=y_units,
                                z_units=z_units,
                                w_units=F403_V_units,
                                # fmt_clabel=lambda cvalue: r"$\mathrm{\$}$"+" {:.1f} ".format(cvalue)+r"$\cdot\mathrm{kg}^{-1}$", # format of contour labels
                                fmt_clabel = lambda cvalue:  f"{round(cvalue,1)}",
                                cmap=CABBI_green_colormap(), # can use 'viridis' or other default matplotlib colormaps
                                cmap_over_color = colors.grey_dark.shade(8).RGBn,
                                extend_cmap='max',
                                cbar_ticks=F403_V_cbar_ticks,
                                z_marker_color='g', # default matplotlib color names
                                fps=fps, # animation frames (z values traversed) per second
                                n_loops='inf', # the number of times the animated contourplot should loop animation over z; infinite by default
                                animated_contourplot_filename='F403_V_animated_contourplot_'+file_to_save, # file name to save animated contourplot as (no extensions)
                                keep_frames=keep_frames, # leaves frame PNG files undeleted after running; False by default
                                axis_title_fonts=axis_title_fonts,
                                clabel_fontsize = clabel_fontsize,
                                default_fontsize = default_fontsize,
                                ...
                                n_minor_ticks = 1,
                                cbar_n_minor_ticks = 1,
                                # comparison_range=[F403_V_w_levels[-2], F403_V_w_levels[-1]],
                                # comparison_range_hatch_pattern='////',
                                units_on_newline = (True, True, False, False), # x,y,z,w
                                )
