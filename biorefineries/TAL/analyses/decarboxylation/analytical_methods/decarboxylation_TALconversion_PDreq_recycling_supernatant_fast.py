...
def solve_decarb_conv_for_mol_ratio(mol_PD_per_mol_TAL):
    def obj_f(x):
        ...
        return H401.ins[0].imol['Acetylacetone']/H401.ins[0].imol['TAL'] - mol_PD_per_mol_TAL
...
U402, M401, S403 = u.U402, u.M401, u.S403
...
def load_S403_recycle_split(spl):
...
H401 = u.H401
get_acetylacetone_presence = lambda: H401.ins[0].imol['Acetylacetone']/H401.ins[0].imol['TAL']

TAL_metrics = [get_product_MPSP, lambda: TAL_lca.GWP, lambda: TAL_lca.FEC, 
               get_TAL_AOC, get_TAL_FCI, get_product_purity, get_acetylacetone_presence]
...
spec_2 = S403_spls = np.linspace(0., 0.99, steps[1]) # titer
...
# spec.load_specifications(TAL_decarb_convs[0], S403_spls[0], PD_prices[0])
...
curr_no = 0
total_no = len(TAL_decarb_convs)*len(S403_spls)*len(PD_prices)
...
for p in PD_prices:
    ...
    file_to_save = f'_{steps}_steps_'+'TAL_decarboxylation_recycling_supernatant_fast%s.%s.%s-%s.%s'%(dateTimeObj.year, dateTimeObj.month, dateTimeObj.day, dateTimeObj.hour, minute)
...
contourplots.animated_contourplot(w_data_vs_x_y_at_multiple_z=results_metric_1, # shape = z * x * y # values of the metric you want to plot on the color axis; e.g., MPSP
                                ...
                                y_data=S403_spls, # y axis values
                                ...
                                #     },
                                additional_points ={(22.5, 0.):('D', 'w', 6)},
