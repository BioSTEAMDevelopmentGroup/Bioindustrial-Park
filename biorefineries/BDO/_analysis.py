...
from BDO.models import BDO_model
...
samples = BDO_model.sample(N=N_simulation, rule='L')
BDO_model.load_samples(samples)
baseline = BDO_model.metrics_at_baseline()
...
parameter_len = len(BDO_model.get_baseline_sample())
parameters = BDO_model.table.iloc[:, :parameter_len].copy()
# Add baseline values to the end
parameters.loc['baseline'] = BDO_model.get_baseline_sample()

Monte_Carlo_results = BDO_model.table.iloc[:, parameter_len::].copy()
Monte_Carlo_percentiles = Monte_Carlo_results.quantile(q=percentiles)
# Add baseline values to the end
Monte_Carlo_results.loc['baseline'] = BDO_model.metrics_at_baseline()

# Note that if only one metric is used, then need to make sure it's a tuple
spearman_metrics = BDO_model.metrics[0:6]
spearman_results = BDO_model.spearman(spearman_metrics)

# Calculate the probabilities of each parameter and the overall scenario
probabilities = {}
for i in range(parameter_len):
    p = BDO_model.get_parameters()[i]
...
with pd.ExcelWriter('E1_baseline_evaluation_88.xlsx') as writer:
    ...
    BDO_model.table.to_excel(writer, sheet_name='Raw data')
...
from BDO.models import BDO_model_IRR
...
np.random.seed(3221)
N_simulation = 100 # 1000
samples = BDO_model_IRR.sample(N=N_simulation, rule='L')
BDO_model_IRR.load_samples(samples)
BDO_model_IRR.evaluate()

parameter_len = len(BDO_model_IRR.get_baseline_sample())
IRR_results = BDO_model_IRR.table.iloc[:, parameter_len::].copy()
...
from BDO.models import IRRs
MSP_indices = [metric.index for metric in BDO_model_IRR.metrics
                if 'Net present value' not in metric.index[1]]
MSP_data = BDO_model_IRR.table[MSP_indices]
...
with pd.ExcelWriter('E2_across_IRR_pure.xlsx') as writer:
    ...
    BDO_model_IRR.table.to_excel(writer, sheet_name='Raw data')
...
from BDO.models import BDO_model_LA_yield, set_LA_yield

'''Evaluate'''
np.random.seed(3221)
N_simulation = 20 # 1000
LA_yield_samples = BDO_model_LA_yield.sample(N=N_simulation, rule='L')
BDO_model_LA_yield.load_samples(LA_yield_samples)
...
LA_yield_data = BDO_model_LA_yield.evaluate_across_coordinate(
...
with pd.ExcelWriter('E3_across_lactic_acid_yield_88.xlsx') as writer:
    ...
    BDO_model_LA_yield.table.to_excel(writer, sheet_name='Raw data')
...
from BDO.models import BDO_model_feedstock, set_feedstock_carbs, prices
...
N_simulation = 1
feedstock_samples_1d = BDO_model_feedstock.sample(N=N_simulation, rule='L')
feedstock_samples = feedstock_samples_1d[:, np.newaxis]
BDO_model_feedstock.load_samples(feedstock_samples)

feedstock_carbs_coordinate = np.linspace(0.4, 0.7, 31) # step = 0.01

feedstock_data = BDO_model_feedstock.evaluate_across_coordinate(
...
with pd.ExcelWriter(name) as writer:
    ...
    BDO_model_feedstock.table.to_excel(writer, sheet_name='Raw data')
...
from chaospy import distributions as shape
from BDO.models import BDO_model_SA_content, set_feedstock_succinic_acid_content

'''Evaluate'''
np.random.seed(3221)
N_simulation = 30 # 1000
SA_content_samples = BDO_model_SA_content.sample(N=N_simulation, rule='L')
BDO_model_SA_content.load_samples(SA_content_samples)
...
SA_content_data = BDO_model_SA_content.evaluate_across_coordinate(
...
with pd.ExcelWriter('E5_across_feedstock_succinic_acid_content_88.xlsx') as writer:
    ...
    BDO_model_SA_content.table.to_excel(writer, sheet_name='Raw data')
...
from chaospy import distributions as shape
from BDO.models import BDO_model_HXN_T_min_app, set_HXN_T_min_app

'''Evaluate'''
np.random.seed(3221)
N_simulation = 60 # 1000
T_min_app_samples = BDO_model_HXN_T_min_app.sample(N=N_simulation, rule='L')
BDO_model_HXN_T_min_app.load_samples(T_min_app_samples)
...
T_min_app_data = BDO_model_HXN_T_min_app.evaluate_across_coordinate(
...
with pd.ExcelWriter('E6_across_T_min_app_88.xlsx') as writer:
    ...
    BDO_model_HXN_T_min_app.table.to_excel(writer, sheet_name='Raw data')
