...
get_jet_vol_ratio = lambda:  get_jet_yield() / get_total_yield()
get_jet_to_eth_ratio = lambda:  get_jet_yield() / get_ethanol_yield()

get_ethanol_energy = lambda:  ethanol.LHV * _GGE_per_J * get_annual_factor()
get_jet_energy = lambda:  jet_fuel.LHV * _GGE_per_J * get_annual_factor()
get_diesel_energy = lambda:  diesel.LHV * _GGE_per_J * get_annual_factor()
get_gasoline_energy = lambda:  gasoline.LHV * _GGE_per_J * get_annual_factor()
...
# Electricity 
# BT.power_utility.production * sys.operating_hours = sys.get_electricity_production() kWh/year
get_excess_power = lambda: BT.power_utility.production * get_annual_factor() - sys.get_electricity_consumption()
electricity_price = price['electricity']
...
get_GWP_material_total = lambda: sys.get_total_feeds_impact('GWP100') 

get_GWP_feedstock = lambda: sys.get_material_impact(feedstock, key='GWP100') / jet_fuel_gal_per_year

get_GWP_other_materials = lambda: get_GWP_material_total() / jet_fuel_gal_per_year - get_GWP_feedstock()

get_GWP_coproducts = lambda: sum([sys.get_material_impact(i, key='GWP') for i in coproducts]) / jet_fuel_gal_per_year

# Total = emission + material
get_GWP_total = get_GWP_emissions_total() + get_GWP_material_total()
...
# Natural gas bought in BT contribution
get_GWP_NG_bought = lambda: sys.get_material_impact(natural_gas, 'GWP100') / jet_fuel_gal_per_year

# Steam utilities contribution
# For heating
get_BT_steam_for_heating = lambda: sum([i.duty for i in BT.steam_utilities]) # in kJ/hr

get_BT_steam_for_electricity = lambda: 3600. * BT.electricity_demand / BT.turbogenerator_efficiency # electricity_demand = -power_utility.rate

get_BT_steam_total = lambda: get_BT_steam_for_heating() + get_BT_steam_for_electricity()

get_steam_frac_heating = lambda: get_BT_steam_for_heating() / get_BT_steam_total

get_steam_frac_electricity = lambda: get_BT_steam_for_electricity() / get_BT_steam_total()

get_steam_frac_cooling = lambda: get_BT_steam_for_electricity() * get_electricity_demand_cooling_frac()

get_steam_electricity_non_cooling = lambda: get_electricity_demand_total() - get_electricity_demand_cooling()

get_steam_frac_electricity_non_cooling = lambda: get_steam_electricity_non_cooling() / get_electricity_demand_total


get_GWP_steam_total = lambda: get_GWP_NG_bought() + get_GWP_emissions_direct_BT()

get_GWP_demand_heating = lambda: get_steam_frac_heating() * get_GWP_steam_total()

get_GWP_demand_cooling = lambda: get_steam_frac_cooling() * get_GWP_steam_total()

get_GWP_demand_electricity_non_cooling = lambda: get_steam_frac_electricity_non_cooling() * get_GWP_steam_total() + get_GWP_electricity_use_total()



metrics.extend((Metric('GWP - total', get_GWP_total, 'kg CO2-eq/gal', 'LCA'),))

metrics.extend((Metric('GWP - feedstock', get_GWP_feedstock, 'kg CO2-eq/gal', 'LCA'),))

metrics.extend((Metric('GWP - other materials', get_GWP_other_materials, 'kg CO2-eq/gal', 'LCA'),))

metrics.extend((Metric('GWP - electricity', get_GWP_electricity_use_total, 'kg CO2-eq/gal', 'LCA'),))

metrics.extend((Metric('GWP - natural gas', get_GWP_NG_bought, 'kg CO2-eq/gal', 'LCA'),)) # bought

metrics.extend((Metric('cGWP - System heating demand', lambda:get_GWP_demand_heating()/get_GWP_total(), 'frac', 'LCA'),))
                       
metrics.extend((Metric('cGWP - System cooling demand',lambda:get_GWP_demand_cooling()/get_GWP_total(), 'frac', 'LCA'),))

metrics.extend((Metric('cGWP - System non-cooling electricity demand', lambda:get_GWP_demand_electricity_non_cooling()/get_GWP_total(), 'frac', 'LCA'),))
