...
from biorefineries.SAF.ATJ.combined.system_whole import F, SAF_sys
from biorefineries.SAF.ATJ._process_settings import price, GWP_CFs, load_preferences_and_process_settings
from biorefineries.SAF._tea import create_ATJ_tea
...
tea_SAF = create_ATJ_tea(sys=sys, unit_transportation_cost=0.009)
...
get_ethanol_conversion  = lambda: ethanol.F_vol * _gal_per_m3 / ((feedstock.F_mass-feedstock.imass['Water'])/1000) # gal ethanol per dry ton biomass
...
metrics = [Metric('Ethanol transportation cost', get_ethanol_transportation_cost, '10^6 $'),
           ...
           Metric('Ethanol conversion', get_ethanol_conversion, 'gal/dry ton biomass'),
...
get_GWP_total = lambda: get_GWP_material_total() + get_GWP_emissions_BT() + get_GWP_emissions_non_BT() + get_GWP_displacement() + get_GWP_ethanol_tranporation()
