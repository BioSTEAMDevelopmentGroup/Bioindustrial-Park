...
#%%
steps = 100
prod_multipliers = np.linspace(0.2, 5.0, steps)
MPSPs, TCIs, AOCs, GWPs, FECs = [], [], [], [], []

for i, pm in zip(range(steps), prod_multipliers):
    print(f'{i+1}/{steps}')
    spec.load_specifications(spec_1=spec.spec_1, spec_2=spec.spec_2, spec_3=spec.baseline_productivity*pm)
    # simulate_and_print()
    system.simulate()
...
def plot_multiple_metrics(x_axis_list, y_axis_list_of_lists, 
                          ...
                          yticks_list=[],
                          save_fig=True,
                          filename='local_sensitivity_productivity.png',
...
#%%
plot_multiple_metrics(spec.baseline_productivity*prod_multipliers,
                      [
                       MPSPs,
                       np.array(TCIs)/1e6,
                       np.array(AOCs)/1e6,
                      ...
                      xlabel=r"$\bfFermentation$"+" " +r"$\bf3HP$"+" " +r"$\bfProductivity$",
