...
def run_TRY_analysis(feedstock, neutralization, yields=None, titers=None, productivities=None, return_YT_arrays=False, plot=False, steps_for_default_YT=50):
    ...
    if feedstock=='glucose':
        ...
        from biorefineries.HP.systems.sugarcane.system_sc_improved_separations import HP_tea, HP_lca, R302, spec, AA, simulate_and_print, get_AA_MPSP
    ...
    spec.reactor.neutralization = neutralization # !!! set neutralization here
    ...
    if not yields:
        yields = np.linspace(0.05, 0.75, steps[0])
    if not titers:
        titers = titers = np.linspace(10., 
                                      200.,
                                        steps[1]) # titer
