...
Water=0.2,
total_flow=127538.3908, #118199.77918567687, # from location model 80MMGal ethanol refinery # 104229.16 original value  # TODO
units='kg/hr',
price=0.08455097) #0.08 $/kg
...
# Acetate=0.0034,
# Protein=0.0058,
# total_flow=127538.3908, # TODO: check this value
# units='kg/hr',
# price=0.08)
...
electricity_price = price['electricity'] # TODO: check if it's changed by state
...
# TODO: MOST METRICS COMMENTED OUT TO SEE IF THE MODEL RUNS FASTER (SEE IF I NEED THEM)
metrics = [
...
# TODO: MOST METRICS COMMENTED OUT TO SEE IF THE MODEL RUNS FASTER (SEE IF I NEED THEM)
# TODO: add these two to the blocs model to get GWP per state
...
def create_states_model(biorefinery):#, feed_price):
    ...
    def MFSP_getter(state):
        def MFSP():
            ...
            # tea.feedstock.price = st_data.loc[state][f'{name} Price (USD/kg)'] # I'M NOT USING PRICE FROM THIS FILE
    ...
    def MFSP_w_inc_getter(state):
        def MFSP():
            ...
            # I'M NOT USING FEEDSTOCK PRICE FROM THIS FILE
    ...
    @model.metric(name='Electricity extra', units='kWh/yr')
    def get_electricity_use():
        return get_excess_power()
    
    @model.metric(name='Energy per year', units='Mj fuel')
    def energy_per_year():
        return _total_energy_per_year()
    
    @model.metric(name='GWP feedstock input', units='gCO2/Mj fuel')
    def feedstock_GWP_input():
        return get_GWP_feedstock_input()
    ...
    if biorefinery == 'cornstover':
        ...
        #                 distribution=shape.Triangle(0.8*0.0994, 0.0994, 1.2*0.0994)) # TODO: Change values
        ...
        # D = shape.Triangle(127538.3908*0.8, 127538.3908, 127538.3908*1.2) # TODO: CHANGE
        ...
        # # TODO: comment
        # D = shape.Triangle(0.078, 0.101, 0.13) # TODO: CHANGE
        ...
        # TODO: comment
        # D = shape.Triangle(-0.122, -0.0497, 0.018) # TODO: CHANGE
...
def evaluate_SS(biorefinery, N=3000, notify_runs=0):#, feed_price = tea.feedstock.price): # TODO: change N
...
# change feedstock price -> tea.feedstock.price = 0.2
# evaluate_SS('cornstover', N=1, notify_runs = 0)

# change feedstock CI -> F.switchgrass.set_CF('GWP100', -0.19)

# there is a problem when running 2 different evaluate_SS(), it keeps the feestock price from
# the previous run. To solve this, run again but using a different N, for example if last run
# used N = 1000, now use N=1, the price problem should be solved
