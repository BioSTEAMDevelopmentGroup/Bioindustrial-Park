...
sizes_of_biorefineries = 500 # In Million Gallons of ethanol per year
# Median size of existing ethanol facilities
# The name is the same as the one used for many sizes

blending_capacity_set = 1 # I'm using 10% blending capacity at jet producers (Set if I want to consider many possibilities)
...
# data = feedstock_delivered_price
...
#%% DISTANCES TO FARMS COLLECTION RADIUS
from statistics import mean 

max_distances_to_farms = np.array([max(dist_sorted[i][:farm_num[i]]) for i in range(1000)]) # for all possible locations

ave_yield_farms = np.array([mean(farm_sizes_sorted[i][:farm_num[i]]) for i in range(1000)])/area *0.8 # in dry tons/ha
