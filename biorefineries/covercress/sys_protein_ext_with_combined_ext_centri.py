...
#error around the mixer M102 to the coverextractor
...
def create_covercress_oil_sys(ins,outs,moisture_content=0.03):# maxmium oil recovery obtained when the cooked seeds was at 3 %
    ...
    loss = 0.5  # 80% loss from miscella, 20% from cake
    ...
    M100 = bst.Mixer('M100', ins=(S100-1, S101-1), outs='hexane_loss_total')
    
    Hex = S100.chemicals.index('Hexane')  # index once
    
    @S100.add_specification(run=False)
    def set_hexane_loss():
        target_loss = 0.5 * feed.F_mass / 1000.0  # kg/hr
        loss_m = loss * target_loss
        loss_c = (1 - loss) * target_loss
    
        fm = loss_m / max(S100.ins[0].imass['Hexane'], 1e-6)
        fc = loss_c / max(S101.ins[0].imass['Hexane'], 1e-6)
...
#refer tutorial mixer settler with model=split does not use lle its just replicates the centrifuge
    ...
    U203 = bst.SolidsCentrifuge(
       ...
       split={
           ...
           'Protein': 0.47,   #0.90 % of 50.1 in first supernatant       
    ...
    U205 = bst.SolidsCentrifuge(
      ...
      split={
          ...
          'Cellulose': 1.0, 'Hemicellulose': 1.0, 'Lignin': 1.0, 'Ash': 1.0,
          # protein: 31% retained in spent solids --> 69% to supernatant refer[27]
          'Protein': 0.94,## 94.1% stays in final solids --> 6.25% to supernatant_2          
          'NaCl': 0, 'NaOH': 0, 'Glucose': 0.0, 'Sucrose': 0.0, 'Hexane': 0.0,
          'OOO': 0.0, 'LLL': 0.0, 'LnLnLn': 0.0, 'SSS': 0.0,'NHP':0.0,
...
def get_SAF_MPSP():
    ...
    return mpsp_kg, mpsp_L   # or return mpsp_L 
