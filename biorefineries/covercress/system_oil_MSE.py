...
def create_covercress_oil_sys(ins,outs,moisture_content=0.03):# maxmium oil recovery obtained when the cooked seeds was at 3 %
   ...
   #cooling of vapours to separate hexane 
    ...
    # U202=bst.MixerSettler('U202', ins=(G201-0,H104-0), outs=('supernatant_1', 'spent_solids'), mixer_data={'tau': 2.0,               # 2 h stir
    #             'agitator_kW_per_m3': 1.0,},model='split',settler_data={
    #                 'split': {
    #                     'Protein': 0.47,         #93.75*of 50 % ie 15 fold of 16 refer[27]    # 47.0% of feed protein -> supernatant_1, protein recovery into supernatant (set from TT8/Y1126) from refr [27]recovered proterin from the TT8 variety is 50.1 % and also from this 15 fold of extracted protein entered into first stage
    ...
    #                     'OOO': 0.01, 'LLL': 0.01,
    #                     'LnLnLn': 0.01, 'SSS': 0.01,'NHP':0.01,
    #                 },
    #             },
    #         )#refer 24
#     U202 = units_oil.ProteinExtractor1('U202',ins=(G201-0, H104-0), outs='saline_extraction_slurry',tau=2.0, 
    
    
   
#              # h [27]
#     # vessel_type, agitator, etc. as needed
# )
    # was: units_oil.ProteinExtractor1(...)
    U202 = SlurryMixTank(
        'U202',
        ins=(G201-0, H104-0),
        outs='saline_extraction_slurry',
        tau=2.0,                 # keep your stage-1 stir time
        
    )
    U202.line = 'Protein extraction stage 1'
#refer tutorial mixer settler with model=split does not use lle its just replicates the centrifuge
    ...
    U203 = bst.SolidsCentrifuge(
       'U203',
       ins=(U202-0),                                 # after the extraction the output is in th eform of slurry which then goes into centrifugation and separate into solids
       ...
       split={
           ...
           # protein: 31% retained in spent solids => 69% to supernatant refer[27] orsince it has only one citation we also refer[42] check the aqueous extraction given recovery is 79-81 %
           'Protein': 0.27,   #0.90 % of 50.1 in first supernatant   or acc to ref [27] 90 % of 69 is 62 so remaian 37 in the spent solids    
    ...
    #     'U204',
    #     ins=(U202-1, H105-0),                     # [0] pellet from stage 1, [1] fresh water
    ...
    #     settler_data={'split': {
    #         'Protein': 0.059,    #10 % of 50 %                         # # 5.9% of the entering (pellet) protein -> supernatant_2 also refer word file-protein extraction
    #         'Water':0.90,'NaCl':0.90,'NaOH':0.90,'Glucose':0.90,'Sucrose':0.90,'Hexane':0.90,
    #         'Cellulose':0.0,'Hemicellulose':0.0,'Lignin':0.0,'Ash':0.0,
    #         'OOO':0.0,'LLL':0.0,'LnLnLn':0.0,'SSS':0.0,'NHP':0.0,
    #     }},
    # )
#     U204 = units_oil.ProteinExtractor2( 'U204',ins=(U203-0, H105-0), outs='saline_extraction_slurry',  tau=2.0, 
...
#     # vessel_type, agitator, etc. as needed
# )
    # was: units_oil.ProteinExtractor2(...)
    U204 = SlurryMixTank(
        'U204',
        ins=(U203-0, H105-0),
        outs='wash_slurry',
        tau=1.0,                 # keep your stage-2 wash time
        
    )
    U204.line = 'Protein extraction stage 2 wash'
    # M111=bst.Mixer('M111',ins=(U204-0,U204-1),outs='mixed_slurry_centrifuge_2')
    U205 = bst.SolidsCentrifuge(
      'U205',
      ins=(U204-0),                                 # after the extraction the output is in th eform of slurry which then goes into centrifugation and separate into solids
      outs=('spent_solids', 'protein_supernatant'), # [0] solids-rich, [1] liquid-rich
      split={
          # insolubles -> spent solids
          'Cellulose': 1.0, 'Hemicellulose': 1.0, 'Lignin': 1.0, 'Ash': 1.0,
          # protein: 31% retained in spent solids --> 69% to supernatant refer[27] since it has only one citation we also refer[42] check the aqueous extraction given recovery is 79-81 %
          'Protein': 0.93,## 93.1% stays in final solids --> 6.25% to supernatant_2          
          'NaCl': 0, 'NaOH': 0, 'Glucose': 0.0, 'Sucrose': 0.0, 'Hexane': 0.0,
          'OOO': 0.0, 'LLL': 0.0, 'LnLnLn': 0.0, 'SSS': 0.0,'NHP':0.0,
...
#         'Enzyme': 1.0,
#         'Cellulose': 1.0, 'Hemicellulose': 1.0, 'Lignin': 1.0, 'Ash': 1.0,
#         'NaOH': 0.0, 'NaCl': 0.0, 'Glucose': 0.0, 'Sucrose': 0.0,
#         'Hexane': 0.0, 'Water': 0.0,
#     },
#     moisture_content=None,    # or 0.10 if you want a wet-cake target
...
#     # =============================================================================
    ...
    M303=bst.Mixer('M303', ins=(K3_l-0,K1_l-0), outs='propane')
    HX302 = bst.HXutility('HX302', ins=M303-0,outs='propane', T=273.15 -40, rigorous=False) # biosteam uses the ethylen as cooling agent so CWP cost  is 0
...
#     D302.check_LHK = False
    ...
    # _PRODUCT_TAU_HR = 7 * 24           
    HX305=bst.HXutility(
        'HX305', ins=D302-0, outs='naphtha_product',
        T=product_storage_T, V=0, rigorous=False,
    )
    HX306 = bst.HXutility(
        'HX306', ins=D303-0, outs='SAF',
        T=product_storage_T, V=0, rigorous=False,
    )
    HX307 = bst.HXutility(
        'HX307', ins=D303-1, outs='green_diesel',
        T=product_storage_T, V=0, rigorous=False,
    )
