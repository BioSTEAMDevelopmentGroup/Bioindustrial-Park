...
total_feed = 100_000        # kg/hr total at gate assumed pennycress 
...
#refer tutorial mixer settler with model=split does not use lle its just replicates the centrifuge
 ...
 # mix extracted and stripped oil
  # =============================================================================s
    ...
    M201 = bst.Mixer('M201', ins=(citric_acid_feed, degum_water_feed), outs='degumming_liquor')
    ...
    def dose_degumming_chemicals():
        ...
        M201.ins[1].imass['Water'] = 0.02 * oil_mass
        M201._run()
    ...
    K301=bst.IsothermalCompressor('K301',ins=S300-0,P=3e6)
    K302=bst.IsothermalCompressor('K302',ins=S300-1,P=4e6)
    ...
    R301 = units_oil.Hydrodeoxygenation(
        ...
        excess_H2=0.3, #TODO check for exact range of excess H2 check for other lit and set uncertainity range
)
    V301=bst.IsenthalpicValve('V301',ins=R301-0, outs=HX_stream_1, P=1e6, vle=False)
    ...
    outs=('gas_to_recovery', 'propane_'),split={
        'Propane': 0.0,     # 90% C3 product (outs[1]) — 
        'CO2': 0.9995,         
        'CO': 0.995,
        'Hydrogen': 0.995,    
        'Water': 0.995,
    ...
    S304=bst.Splitter('S304', ins=S303-0, outs=('acid_gas','H2_recycle'), split={'Water':0.995, 'CO':0.999,'CO2':0.999})
...
#     P301=bst.IsothermalCompressor('P301',ins=F303-0, P=101325) #since the input is gas so we use isothermal compressor
    ...
    S305 = bst.Splitter( #why splitter because for R302 the liquid molar enthalpy model failed to  find molar enthalpy for hexane at 523
    'S305',
    ins=F301-1,
...
#     # =============================================================================
    ...
    split={
        ...
        'Propane': 0.50,      # remaining propane split 
        'Heptane': 0.99,
    ...
    M303=bst.Mixer('M303', ins=(S303-1,F305-0), outs='propane')
    ...
    S301 = bst.SolidsSeparator(
    'S301',
    ins=F304-1,
    outs=('spent_catalyst', 'decat_liquid'),
    split={'Pt/Al2O3': 1},#assume all calatayst is separated if any traces is present in the downstream biosteam model crashes because the model fails to find gas enthalpy of catalsyt
)
    ...
    LHK=('Heptane', 'Octane'),
    Lr=0.99,   # most heptane to distillate
    Hr=0.99,   # almost no octane in overhead
    P=101325,
    k=1.2,
)
    ...
    LHK=('IsoNonane', 'Decane'),        # C9 / C10 cut naphtha C5-C9
    Lr=0.52,                         # nonanerefer [4] table 1 #TODO check actual distillate composition from industry
    Hr=0.99,                         # decane mostly bottoms
    P=101325,
    k=1.5,
)
    ...
    outs=(SAF, green_diesel),
    LHK=('IsoPentadecane','Heptadecane'),  # C16 / C17 cut
    Lr=0.99,                         # C16 and lighter → distillate (SAF) C8-C16
    Hr=0.99,                         # C17 bottoms (diesel)C17-C22 
...
saf_tea.IRR = 0.10
saf_tea.operating_days = 60 # availbility of covercress is in may-june so max 60 days
...
def get_feedstock_MFPP():
    return solve_scenario(feedstock, label='Scenario B: feedstock MFPP')
...
def print_tea_summary(MSP_SAF, MFPP):
    ...
    print(f'\nSAF MPSP            : ${MSP_SAF:.4f}/kg')
    print(f'Market SAF          : ${price["SAF"]:.4f}/kg')
...
def run_tea():
    ...
    MSP_SAF = get_SAF_MPSP()
    MFPP = get_feedstock_MFPP()
    print_tea_summary(MSP_SAF, MFPP)
    return MSP_SAF, MFPP
