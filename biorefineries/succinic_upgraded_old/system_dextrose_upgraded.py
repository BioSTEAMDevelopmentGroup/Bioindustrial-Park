...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov  8 11:07:40 2025
...
l_phenylalanine=bst.Stream('l_phenylalanine')
exp = math.exp
...
def create_succinic_sys(ins, outs):
    ...
    R302 = units_adsorption_upgraded.CoFermentation('R302', 
    ...
    R303 = units_adsorption_upgraded.SeedTrain('R303', 
                           ins=(S302-0, S303-0), 
    ...
    T301 =units_adsorption_upgraded.SeedHoldTank('T301', ins=R303-0, outs=1-R302)
    ...
    def A301_spec():
        ...
        A301.outs[1].phase='g'
    K301 =units_adsorption_upgraded.IsothermalCompressor('K301', ins=A301-1, outs=('recycled_CO2'), 
                                    P=3e7, 
                                    # vle=True,
                                    eta=0.6,
    ...
    K302 =units_adsorption_upgraded.IsothermalCompressor('K302', ins='atmospheric_air', outs=('pressurized_air'), 
...
# No carbon constraint: Allows flexibility in modeling mixed carbon sources.
    ...
    H402 =units_adsorption_upgraded.HXutility('H402', ins=S401-1, outs = ('broth_to_adsorbtion',), T=20. + 273.15)
    ...
    T400 =units_adsorption_upgraded.CausticStorage('T400', ins = NaOH, outs = 'fresh_NaOH')
    ...
    make_up_AC= Stream('Make_Up_adsorbent',units='kg/hr', price=price['ActivatedCarbon'])
    ...
    def M400_spec():
        ...
        M400.ins[0].phase = 'l'
        M400.ins[2].phase = 'l'
        required_NaOH=A401.outs[1].imass['NaOH']
        M400.ins[1].imass['NaOH']= required_NaOH
    ...
    A401=units_adsorption_upgraded.SingleComponentAdsorptionColumn('A401',ins=(H402-0,M400-0,make_up_AC), outs=('broth_post_adsorption','spent_fluid','spent_adsorbent' ),
                                    ...
                                    # (5.4,64.4)
                                        ...
                                        k=1.2,
                                        void_fraction = 0.5, 
                                        # the min void fraction for this sytem requires 0.5 without excessive pressure drop which then wont affect the flow rate 
                                        k_regeneration=360,
...
#other sources shows 8-30 % losses asrer 4 cycles 
    ...
    R401 =units_adsorption_upgraded.AcidulationReactor('R401', ins=(A401-0, sulfuric_acid_R401),
    ...
    S405 =units_adsorption_upgraded.GypsumFilter('S405', ins=R401_P-0,
    ...
    F401.target_concentration = 0.25 # ~than 250 g/L,(5X  *titer value after conc approx(0.30=4*78.31))
    ...
    C401 =units_adsorption_upgraded.SuccinicAcidCrystallizer('C401', ins=F401_P-0, outs=('C401_0',), 
    ...
    F402.target_concentration = 0.25 #considered 4X titer value
    # < than 250 g/L,(5X  *titer value after conc approx 60 g/l from the experimet the maximum concentration at which experimental data was collected
   ...
   #TODO check for target conc
    ...
    C402 =units_adsorption_upgraded.SuccinicAcidCrystallizer('C402', ins=F402_P-0, outs=('C402_0',), 
                                   target_recovery=0.95,
                                   tau=6,
    ...
    F403.target_concentration = 0.25 # ~250 g/L, the maximum concentration at which experimental data was collected
    ...
    C403 =units_adsorption_upgraded.SuccinicAcidCrystallizer('C403', ins=F403_P-0, outs=('C403_0',), 
                                   target_recovery=0.95,
                                   tau=6,
                                   T_range=(273.15+0.5, 373.15-2.),
                                   N=4,
    ...
    T602 =units_adsorption_upgraded.LimeStorage('T604', ins=base_fermentation, 
    ...
    T603 =units_adsorption_upgraded.SulfuricAcidStorage('T602', ins=sulfuric_acid_acidulation,
...
def load_titer_with_glucose(titer_to_load, set_F301_V=None):
    ...
    # F301_lb=0.01 for ld yield and productivity when yield and productivity changes to new F301_lb=0.5
...
for i in unit_groups: i.autofill_metrics(shorthand=False, 
                                         ...
                                         material_cost=True)
# TODO electricity production=False changed to True
...
#                      y_ticks=[-175,-150,-125, -100, -75, -50, -25, 0, 25, 50, 75, 100, 125, 150, 175, 200, 225], 
