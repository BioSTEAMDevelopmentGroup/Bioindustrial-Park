...
[6]Environmental impacts analysis of titanium sponge production using Kroll process in China
[7] A global life cycle assessment for primary zinc production
...
NaOH_price = 0.31 * _chemical_index_2025to2016
# Magnesium sulfate
...
# mean of 0.0.25(range 0.20 to 0.30)	$/kg in 2007 $	
# using GDP= 2007 to 2026 ~1.15
#TODO mean of 0.505 (range of 0.460-0.549) $/kg when converted to 2016$ need to check for this chemical
# https://www.made-in-china.com/video-channel/yifengtuo_zmWpgIJCqHkY_Price-of-Magnesium-Chloride-Hexahydrate-Industrial-Grade.html                     
magnesium_chloride_hexahydrate_price = 0.43 #2016 price
# * _chemical_index_2025to2016
# https://catcost.chemcatbio.org/materials-library
#zinc sulphate heptahydarate
# mean -0.60 (range- 0.50 to 0.70 per kg)
# https://www.tradeindia.com/manufacturers/zinc-sulphate-heptahydrate.html
zinc_sulfate_heptahydrate_price = 0.75  # 2016 price
# *_chemical_index_2025to2016
# https://catcost.chemcatbio.org/materials-library
# Darco G-60 Activated Carbon (experimentally used)
# Belhamdi, B., Merzougui, Z., Trari, M., & Addoun, A. (2016). A kinetic, equilibrium and thermodynamic study of l-phenylalanine adsorption using activated carbon based on agricultural waste (date stones). Journal of Applied Research and Technology, 14(5), 354-365. https://doi.org/10.1016/j.jart.2016.08.001
# Note: Study used agricultural waste AC but Darco G-60 is the commercial reference
# Darco G-60 pricing: $460/kg (1kg size) from USBio.net
# USBio.net reference: https://www.usbio.net/biochemicals/136542/activated-charcoal-darco-g-60
# Price calculation: $460/kg × (102.5/126.6) = $372.20/kg in 2016$
# But DARCO no using instead conventional AC is used
ActivatedCarbon_price= 2 * _chemical_index_2025to2016  # $372.20/kg - Darco G-60 activated carbon
# TODO update the CFs for AC
...
price = {
         ...
         'ActivatedCarbon': ActivatedCarbon_price,
         'make_up_AC': ActivatedCarbon_price,
         ...
         'MagnesiumChlorideHexahydrate': magnesium_chloride_hexahydrate_price,
         'Glucose': glucose_price,
         'Zinc Sulfate Heptahydrate': zinc_sulfate_heptahydrate_price,
...
GWP_CFs = {
    ...
    'MagnesiumChlorideHexahydrate': 1.5,  # estimated from MgCl2
    'Zinc Sulfate Heptahydrate': 2.35,  # ecoinvent direct value
    'ActivatedCarbon': 3.27,
# TODO update the value for CA, two nutrients        # 
# carbon GWp from econvent is 3.27 Kg CO2eq
#zinc sulpide 2.06 Kg CO2eq
#magenium chloride 0 Kg CO2 eq=Many magnesium chloride datasets in ecoinvent represent MgCl₂ as a byproduct or secondary output from another production process—such as hafnium or titanium sponge manufacturing—where most impacts are allocated to the main product, not to MgCl₂. I
    }
...
FEC_CFs = {
    ...
    #TODO : check the correct FEC value for crbon
    'NaOH': 29,
    ...
    'MagnesiumChlorideHexahydrate': 10,  # As detailed in source [6], magnesium chloride (MgCl₂) is co-produced during the titanium sponge manufacturing process.
    'Zinc Sulfate Heptahydrate': 30,  # refer [7] no source explicitly mention the FEC for ZInc sulphate but, as Zinc extraction process consumes almost 37.5MJ/Kg of FE and from greet 30 Mj/kg
     'ActivatedCarbon': 6.12,     # activated carbon: proxy charcoal(same source)
