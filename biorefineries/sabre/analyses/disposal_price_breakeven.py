...
"""
disposal_price_breakeven.py
----------------------------
For the four standalone SaBRe flowsheets (the 'integrated' flowsheet is
excluded, same as `msp_comparison.py`), solve the solid-waste disposal
price ($/kg) that hits the target IRR (data/tea.yaml `tea.IRR`), holding
...
disposal_solid's price is solved holding every product's price fixed at its
own tea.yaml baseline (biostimulant_product included, priced as a
co-product on each AD pathway's combined TEA, same as `msp_comparison.py`'s
`credit_tipping_fee=False` basis).

Each pathway's solid-waste streams share a single flat tea.yaml
`price.disposal_solid.baseline` price today (no per-stream differentiation
-- see data/tea.yaml's `price.disposal_solid.note` for the per-stream
COD-basis estimates this flat value stands in for). `TEA.solve_price()`
natively accepts multiple streams and solves for the one common price that
zeroes system NPV at target IRR, so that's used directly instead of
assuming/solving each waste stream separately.

A solved price close to or more negative than the tea.yaml baseline means
the pathway is already profitable paying that disposal cost; a solved
price less negative than (or above) baseline means the pathway can't hit
target IRR at the baseline disposal cost and needs a cheaper (or revenue-
generating) outlet for its solid waste.

A second figure (run_disposal_vs_price_four_systems()) sweeps this same
break-even disposal price across each pathway's own product over its own
data/tea.yaml price range (`price.biostimulant.range`, `price.vfa.range`,
`price.microbial_oil.range`, `price.biomethane_mmbtu.range`), all four
pathways as one figure/one set of axes. Since each pathway's product has
a different unit and price scale, both axes are normalized to % of that
pathway's own tea.yaml baseline (product price on x, disposal_solid price
on y) -- so (100%, 100%) always means "everything at its tea.yaml
baseline," for every line.

Run from the repo root:
    python biorefineries/sabre/analyses/disposal_price_breakeven.py
"""
...
DISPOSAL_SOLID_BASELINE = _TEA_PRICE["disposal_solid"]["baseline"]
DISPOSAL_SOLID_RANGE = _TEA_PRICE["disposal_solid"]["range"]
...
# Solid-waste stream names (sys.flowsheet.stream) for each pathway. pressed_cake
# is deliberately excluded from every AD pathway: create_*_system() always sets
# its price, but once it's consumed by that pathway's own Mill it's an internal
# stream, not a system boundary stream -- so that price is inert for TEA
...
def solve_disposal_breakeven(sys: bst.System, label: str) -> dict:
    """
    Solve the common solid-waste disposal price ($/kg) that zeroes this
    system's NPV at its TEA's target IRR, holding every already-priced
    product/reagent/other-waste stream fixed at its current price (i.e.
    the data/tea.yaml baseline each create_*_system() sets by default).
    """
    fw = sys.flowsheet.stream
    waste_streams = [getattr(fw, name) for name in WASTE_STREAMS_BY_LABEL[label]]

    breakeven_usd_per_kg = sys.TEA.solve_price(waste_streams)

    annual_hours = sys.TEA.operating_days * 24
    annual_waste_kg = sum(float(s.F_mass) for s in waste_streams) * annual_hours

    product = getattr(fw, PRODUCT_STREAM_BY_LABEL[label])

    return {
        "label": label,
        "waste_stream_names": WASTE_STREAMS_BY_LABEL[label],
...
def run_disposal_price_breakeven() -> None:
    results = []
    ...
    ax.axhline(
        DISPOSAL_SOLID_BASELINE, color="black", linewidth=1.0, linestyle="--", zorder=2,
    )
    ax.text(
        -0.5, DISPOSAL_SOLID_BASELINE,
        f"tea.yaml baseline (${DISPOSAL_SOLID_BASELINE:.2f}/kg) ",
        ha="left", va="bottom", fontsize=8, color="#444444",
    )

    ax.set_ylabel("Break-even solid-waste disposal price ($/kg)")
    ax.set_title(
        "Solid-waste disposal price for break-even (target IRR), products at tea.yaml baseline",
        fontsize=10,
    )
    ax.yaxis.set_major_formatter(mticker.FormatStrFormatter("$%.2f"))
    ax.grid(axis="y", linewidth=0.4, color="#D3D1C7", zorder=0)
    ax.axhline(0, color="black", linewidth=0.8, zorder=3)
    lo, hi = min(0, min(vals), DISPOSAL_SOLID_BASELINE), max(0, max(vals), DISPOSAL_SOLID_BASELINE)
    pad = (hi - lo) * 0.22 or 1
    ax.set_ylim(lo - pad, hi + pad)
    ax.tick_params(axis="x", labelrotation=15)

    fig.tight_layout()
    out_path = OUT / "fig_disposal_price_breakeven_four_systems.png"
    ...
    # ── Summary table ────────────────────────────────────────────────────
    print(
        f"\nSolid-waste disposal break-even price by pathway "
        f"(products at tea.yaml baseline; tea.yaml disposal_solid baseline = "
        f"${DISPOSAL_SOLID_BASELINE:.4f}/kg, range {DISPOSAL_SOLID_RANGE}):"
    )
    header = (
        f"  {'Pathway':<38}{'Product @ price':<26}"
        f"{'Break-even ($/kg)':>20}{'Headroom vs baseline':>22}{'Annual waste (t/yr)':>22}"
    )
    print(header)
    for r in all_results:
        ...
        print(
            ...
            f"{r['annual_waste_kg'] / 1000:>21,.1f}"
        )
    print(
        "  (Headroom > 0: pathway needs a less-negative/positive disposal price "
        "than tea.yaml baseline to hit target IRR -- i.e. it's short of target "
        "IRR at baseline. Headroom < 0: pathway already clears target IRR at "
        "baseline and could absorb a costlier disposal price.)"
    )
...
SWEEP_CONFIGS = (
    {
        ...
        "x_range": tuple(BIOSTIMULANT_RANGE),
        "x_baseline": BIOSTIMULANT_BASELINE,
...
def sweep_disposal_vs_product_price(cfg: dict, n_points: int) -> dict:
    """
    Build cfg['label']'s system once, simulate once, then sweep
    cfg['product_stream_name']'s price over cfg['x_range'] (converted to
    $/kg via cfg['to_usd_per_kg'] if given) and solve the break-even solid-
    waste disposal price at each point via solve_disposal_breakeven()'s
    ...
    for x in x_values:
        ...
        breakevens.append(sys.TEA.solve_price(waste_streams))
...
def run_disposal_vs_price_four_systems(n_points: int = 16) -> None:
    """
    ...
    break-even solid-waste disposal price at each point. The x-axis
    (product price) is normalized to a % of that pathway's own tea.yaml
    baseline -- despite having different products/units/price scales, all
    four pathways then share one x-axis, with 100% always meaning "this
    ...
    actual break-even disposal price ($/kg) -- common to every pathway
    already, so no normalization needed there.
    """
    sweep_results = [sweep_disposal_vs_product_price(cfg, n_points) for cfg in SWEEP_CONFIGS]
    ...
    ax.axhline(DISPOSAL_SOLID_BASELINE, color="black", linewidth=1.0, linestyle="--", zorder=2)
    ax.text(
        ax.get_xlim()[0], DISPOSAL_SOLID_BASELINE,
        f" tea.yaml disposal_solid baseline (${DISPOSAL_SOLID_BASELINE:.2f}/kg) ",
        ha="left", va="bottom", fontsize=8, color="#444444",
    )
    ...
    ax.set_ylabel("Break-even disposal price ($/kg)")
    ax.set_title(
        "Break-even solid-waste disposal price vs. product selling price\n"
    ...
    fig.tight_layout()
    out_path = OUT / "fig_disposal_price_vs_selling_price_pct_four_systems.png"
    fig.savefig(out_path, bbox_inches="tight")
...
if __name__ == "__main__":
    run_disposal_price_breakeven()
    run_disposal_vs_price_four_systems()
