...
def get_succinic_acid_solubility_gpL(T):
    return 29.098*math.exp(0.0396*(T-273.15))
from biorefineries import succinic
from biorefineries.succinic import system_glucose

from biorefineries.succinic.system_glucose import succinic_sys
...
f_old = succinic.system_glucose.flowsheet
s = f_old.stream
u = f_old.unit
...
# Store upstream subsystem globally so it can be accessed later
new_sys_up = None
...
def create_new_adsorption_units_only_sys(ins, outs):
    ...
    new_sys_up = succinic_sys.from_units(ID = 'upstream_sys', units = [f_old.U101, f_old.M201, f_old.U302, f_old.S301, f_old.H302, f_old.P303,\
                                                                      f_old.F301, f_old.H301,f_old.M304, f_old.M304_H, f_old.S302, f_old.S303, f_old.R303,\
                                                                 f_old.M305, f_old.T301,f_old.K301, f_old.K302, f_old.H401,f_old.M401])
    feedstock-0-f_old.U101
    ...
    R302_1 = units_adsorption_upgraded.CoFermentation('R302_1', 
                                    ins=(
                                        f_old.S302-1, 'seed', CSL, f_old.S303-1, base_neutralization, 
                                         
                                         # M300_R-0, 
                                         f_old.K301-0, # CO2 recycle (K301-0)
    ...
    def R302_1_spec(): # note: effluent always has 0 CSL
     # R302.show(N=100)
        ...
        if R302_1 .ins[2].F_mol:
            R302_1 .ins[2].F_mass*=1./(1-f_old.S302.split[0])
            R302_1 ._run()
            f_old.S303.simulate()
            f_old.K302.specifications[0]()
    (f_old.S302-0,f_old.S303-0)-f_old.R303
    
    (R302_1 -1,f_old.R303-1)-f_old.M305
    f_old.R303-0-0-f_old.T301
    f_old.T301-0-1-R302_1 
    ...
    # Note: f_old.makeup_MEA_A301 might be used, but we need to use the new one
    # Check if f_old has makeup_MEA_A301 and update reference
    A301_1 = bst.AmineAbsorption('A301_1', ins=(f_old.M305-0, makeup_MEA_A301, 'A301_makeup_water'), outs=('absorption_vent', 'captured_CO2'),
                               CO2_recovery=0.52)
    ...
    A301_1-1-0-f_old.K301
       
    (f_old.CO2_fermentation)-f_old.S303  
    # f_oldK301-0-5-R302
    # f_oldK302-0-8-R302
    R302_1 -0-0-f_old.H401
    f_old.H401-0-0-f_old.M401
    S401_1= bst.SolidsCentrifuge('S401_1', ins=f_old.M401-0, 
    ...
    succinic_downstream = succinic_sys.from_units(ID = 'downstream_sys', units= [f_old.R401, f_old.R401_P, f_old.S405, f_old.M404, f_old.F401, f_old.F401_P, f_old.C401,\
                                                                        f_old.S402, f_old.F402, f_old.F402_P, f_old.C402, f_old.S403, f_old.F403, f_old.F403_P,\
                                                                        f_old.C403, f_old.S404, f_old.M402, f_old.F404, f_old.M403,\
                                                                        f_old.S406, f_old.M501, f_old.M502, f_old.U501, f_old.R501,\
                                                                        f_old.M503, f_old.R502, f_old.R503,\
                                                                        f_old.S501, f_old.M504, f_old.C501, f_old.M505, f_old.U502,\
                                                                        f_old.M510, f_old.M405, f_old.T601, f_old.T601_P,\
                                                                        f_old.T602, f_old.T603,\
                                                                        f_old.BT701,\
                                                                        f_old.CT801, f_old.CWP802, f_old.CIP901,\
                                                                         f_old.ADP902, f_old.FWT903,  f_old.PWC904])
    ...
    (A401-0,f_old.sulfuric_acid_R401)-f_old.R401
    
 
  
    # Create adsorption subsystem with new units
    sys_adsorption = succinic_sys.from_units(ID='sys_adsorption', units=[R302_1, A301_1, S401_1, H402, T400, M400, A401, S400_W, F400, F400_P])
    
    # Collect all units from subsystems and new units
    combined_units = list(new_sys_up.units)
...
# Register system - it should already be registered with ID 'succinic_sys_new'
# when created via SystemFactory. ProcessSpecification now checks for 
# succinic_sys_new first, so it should find it.
# Don't overwrite f1.system as it's the system registry, not the current system
# The system is accessible via flowsheet('succinic_sys_new') which ProcessSpecification uses
f1 = bst.main_flowsheet
# The system is already registered when created via SystemFactory
# We don't need to set f1.system as that would overwrite the registry

# Register all units from the system into the flowsheet's unit registry
# (f1 already defined above)
...
all_system_units = get_all_units_from_system(succinic_sys_new)

# Register all units in the new flowsheet's unit registry
for unit in all_system_units:
    f1.unit.data[unit.ID] = unit

# Also register ALL units from old flowsheet that are used in the system
# Get units from new_sys_up and succinic_downstream which contain old flowsheet units
def get_units_from_subsystem(subsystem):
    """Get all units from a subsystem, including nested subsystems"""
    units = list(subsystem.units)
    for sub in subsystem.subsystems:
        units.extend(get_units_from_subsystem(sub))
    return units

# Register units from upstream subsystem
for sys in succinic_sys_new.subsystems:
    if sys.ID == 'upstream_sys':
        for unit in get_units_from_subsystem(sys):
            f1.unit.data[unit.ID] = unit
    elif sys.ID == 'downstream_sys':
        for unit in get_units_from_subsystem(sys):
            f1.unit.data[unit.ID] = unit

# Also register units from old flowsheet that might be referenced but not in subsystems
# This ensures all old flowsheet units used anywhere are accessible
old_units_to_register = [
    'U101', 'M201', 'U302', 'S301', 'H302', 'P303', 'F301', 'H301', 
    'M304', 'M304_H', 'S302', 'S303', 'R303', 'M305', 'T301', 
    'K301', 'K302', 'H401', 'M401', 'R401', 'R401_P', 'S405', 
    'M404', 'F401', 'F401_P', 'C401', 'S402', 'F402', 'F402_P', 
    'C402', 'S403', 'F403', 'F403_P', 'C403', 'S404', 'M402', 
    'F404', 'M403', 'S406', 'M501', 'M502', 'U501', 'R501', 
    'M503', 'R502', 'R503', 'S501', 'M504', 'C501', 'M505', 
    'U502', 'M510', 'M405', 'T601', 'T601_P', 'T602', 'T603',
    'BT701', 'CT801', 'CWP802', 'CIP901', 'ADP902', 'FWT903', 'PWC904'
]
for unit_id in old_units_to_register:
    if hasattr(u, unit_id):
        unit = getattr(u, unit_id)
        # Register even if already registered to ensure it's accessible
        f1.unit.data[unit.ID] = unit
...
# T601_P is in the downstream system from old flowsheet
product_stream = u_1.T601_P.outs[0] if hasattr(u_1, 'T601_P') else s_1.SuccinicAcid
# Register the product stream with ID 'SuccinicAcid' in the flowsheet for easy access
if not hasattr(s_1, 'SuccinicAcid') or s_1.SuccinicAcid is not product_stream:
    # Register product_stream as SuccinicAcid in the stream registry
...
spec = ProcessSpecification(
    ...
    pre_conversion_units = new_sys_up.split(u_1.H301.outs[0])[0] if new_sys_up else succinic_sys_new.split(u_1.H301.outs[0])[0],
...
# (CSL, nutrients, MEA, Lime) that don't have source units
conversion_group = unit_groups_dict.get('conversion')
if conversion_group:
    # Create new getter that sums all material costs directly from streams
    def conversion_material_cost():
        # Calculate material cost for all streams with prices in conversion units
        # This includes external feeds (CSL, nutrients, MEA) that might be missed by get_material_cost()
        total_cost = 0
        ...
        # Lime consumption is calculated during _run() and added to base stream
        try:
            if hasattr(u_1, 'R302_1'):
                ...
                if (hasattr(r302_1, 'pH_control') and r302_1.pH_control and 
                    ...
                    # Get base stream (typically at index 4)
                    lime_mass_from_stream = 0
                    if len(r302_1.ins) > 4:
                        base_stream = r302_1.ins[4]
                        # Check if Lime is present in base stream after unit has run
                        if base_stream and hasattr(base_stream, 'imass'):
                            try:
                                # Check if Lime is in the chemicals
                                if 'Lime' in base_stream.chemicals:
                                    lime_mass_from_stream = base_stream.imass['Lime']
                                    if lime_mass_from_stream > 0 and 'Lime' in price:
                                        lime_cost = price['Lime'] * lime_mass_from_stream
                                        if lime_cost > 0.01:
                                            total_cost += lime_cost
                            except (KeyError, AttributeError):
                                pass
                            
                            # Alternative: Calculate Lime from succinic acid production if base stream is empty
                            # Lime consumption = (SuccinicAcid mol) / (lime_pH_control_rxns.X[1]) * safety_factor
                            if lime_mass_from_stream == 0 and r302_1.outs and len(r302_1.outs) > 0:
                                ...
                                if hasattr(effluent, 'imol') and 'SuccinicAcid' in effluent.chemicals:
                                    try:
                                        ...
                                        if succinic_mol > 0 and hasattr(r302_1, 'mol_lime_per_acid_pH_control'):
                                            # Lime mol = succinic_mol / X[1] * safety_factor
                                            # From unit code: base.imol['Lime'] = self.mol_lime_per_acid_pH_control * (effluent.imol['SuccinicAcid']/self.lime_pH_control_rxns.X[1]) * self.neutralization_safety_factor
                                            ...
                                            if lime_mol > 0 and 'Lime' in chems:
                                                lime_mass_calc = lime_mol * chems.Lime.MW
                                                lime_cost_calc = price['Lime'] * lime_mass_calc
                                                if lime_cost_calc > 0.01:
                                                    total_cost += lime_cost_calc
    ...
    # Override the material cost metric
...
# Verify conversion material cost after simulation
if conversion_group:
