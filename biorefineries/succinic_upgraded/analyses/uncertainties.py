...
from biorefineries.succinic_upgraded.models_dextrose import model
...
models = succinic_upgraded.models_dextrose
# from . import models
...
index = [textwrap.fill(i.element_name + ': ' + i.name, width=30) for i in model.parameters]
...
fig[0].set_size_inches(10, len(index) * 0.4)
ax = fig[0].axes[0]
...
# Dynamically adjust figure height based on number of parameters
fig[0].set_size_inches(10, len(index) * 0.4)

# Access the axes and improve label spacing
ax = fig[0].axes[0]
ax.tick_params(axis='y', pad=10, labelsize=8)
