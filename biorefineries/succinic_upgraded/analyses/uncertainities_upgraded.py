...
from biorefineries import succinic_upgraded
from biorefineries.succinic_upgraded.models_upgraded import model
# models = succinic_upgraded.get_models()
#TODO  check this later--> models = succinic_upgraded.models_dextrose changed the line succinic_upgradedget_models()
models = succinic_upgraded.models_upgraded
# from . import models
#from biosteam.utils import TicToc
#TODO TicTco does not exist in the utils
...
class Timer:
    ...
    def toc(self):
        if self.start_time is None:
            print("Timer has not been started with tic()")
            return
...
chdir = os.chdir
succinic_filepath = succinic_upgraded.__file__.replace('/__init__.py', '')
...
N_simulations_per_mode = 3000 # 2000
...
for i in range(len(modes)):
    ...
    results_dict['Baseline']['GWP Breakdown'][mode] = {
        ...
        'magnesium chloride hexahydrate': material_GWP_breakdown['MagnesiumChlorideHexahydrate'],
        'zinc sulfate heptahydrate': material_GWP_breakdown['Zinc Sulfate Heptahydrate'],
        ...
        'natural gas\n(for steam generation)': lca.natural_gas_GWP,
        'natural gas\n(product drying)': material_GWP_breakdown['CH4'],
        'net electricity': lca.net_electricity_GWP,
        'direct non-biogenic\nemissions': lca.direct_emissions_GWP,
        'Activated ':material_GWP_breakdown['ActivatedCarbon'],
      
        }   
#TODO lca.ng_GWt o lca.natural_gas_GWP,
    ...
    results_dict['Baseline']['FEC Breakdown'][mode] = {
        ...
        'magnesium chloride hexahydrate': material_FEC_breakdown['MagnesiumChlorideHexahydrate'],
        'zinc sulfate heptahydrate': material_FEC_breakdown['Zinc Sulfate Heptahydrate'],
        'ActivatedCarbon':material_FEC_breakdown['ActivatedCarbon'],
...
##TODO lca.ng_GWt o lca.natural_gas_GWP, and also removed H3PO4
...
MPSP_units = r"$\mathbf{\$}\cdot\mathbf{kg}^{-1}$"
GWP_units = r"$\mathbf{kg}$"+" "+ r"$\mathbf{CO}_{2}\mathbf{-eq.}\cdot\mathbf{kg}^{-1}$"
FEC_units = r"$\mathbf{MJ}\cdot\mathbf{kg}^{-1}$"
#%% Uncertainty
...
contourplots.box_and_whiskers_plot(uncertainty_data=MPSP_uncertainty, 
                          ...
                          baseline_marker_shapes=["p"],
                          baseline_marker_sizes=[10],
...
contourplots.box_and_whiskers_plot(uncertainty_data=GWP_uncertainty, 
                          ...
                          y_label=r"$\mathbf{GWP}_{\mathbf{100}}$",
                          y_units=GWP_units,
...
contourplots.stacked_bar_plot(dataframe=df_GWP_breakdown, 
                 ...
                 y_label =r"$\mathbf{GWP}_{\mathbf{100}}$" +" "+ r"$\mathbf{Breakdown}$",
                 y_units = "%", 
...
contourplots.stacked_bar_plot(dataframe=df_FEC_breakdown, 
                 ...
                 y_label=r"$\mathbf{FEC}$" +" "+ r"$\mathbf{Breakdown}$", 
                 y_units = "%" , 
...
index = [textwrap.fill(i.element_name + ': ' + i.name, width=15) for i in model.parameters]
fig = bst_plots.plot_spearman_1d(results_dict['Sensitivity']['Spearman']['MPSP'][mode],
                           index=index,
                           name='MPSP - Pilot scale [fedbatch] '+"["+MPSP_units+"]", color="#A97802",
                           xlabel= "Spearman's "+rho+ " with "+ "MPSP")

fig[0].set_size_inches(10, len(index) * 0.35)
ax = fig[0].axes[0]
...
# ax = fig[0].axes[0]  # assuming one subplot

# # Increase space between y-axis labels
# ax.tick_params(axis='y', pad=10)  # adds space between ticks and labels
...
index = [textwrap.fill(i.element_name + ': ' + i.name, width=15) for i in model.parameters]
# Generate the Spearman plot for GWP100
fig = bst_plots.plot_spearman_1d(
    results_dict['Sensitivity']['Spearman']['GWP100a'][modes[0]],
    index=index,
    name='GWP100 - Pilot scale [fedbatch] [' + GWP_units + ']',
    color='#607429',
    xlabel="Spearman's " + rho + " with GWP100"
)
...
#                            index=[i.element_name + ': ' + i.name for i in model.parameters],
#                            name='GWP100 - Pilot scale [fedbatch] '+"["+GWP_units+"]", color='#607429',
#                            xlabel= "Spearman's "+rho+ " with "+ "GWP100")
...
fig = bst_plots.plot_spearman_1d(results_dict['Sensitivity']['Spearman']['FEC'][modes[0]],
                           index=[i.element_name + ': ' + i.name for i in model.parameters],
                           name='FEC - Pilot scale [fedbatch] '+"["+FEC_units+"]", color='#A100A1',
                           xlabel= "Spearman's "+rho+ " with "+ "FEC")
...
fig[0].savefig(mode+'FEC-Spearman.png', dpi=600, bbox_inches='tight',
            facecolor=fig[0].get_facecolor(),
            transparent=False)
#TODO changed modes[] accordingly 
#TODO changed xlabel_fn paprmater in plot_spearman_1D results to xlabel
