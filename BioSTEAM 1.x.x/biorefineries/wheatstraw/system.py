...
from biorefineries.wheatstraw.process_settings import price, ethanol_density_kggal
from biorefineries.wheatstraw.chemicals import wheatstraw_chemicals, get_grouped_chemicals, chemical_groups
from biorefineries.wheatstraw.tea import WheatstrawTEA
...
Ethanol_MW = wheatstraw_chemicals.Ethanol.MW
Water_MW = wheatstraw_chemicals.Water.MW
...
pretreatment_chemicals = wheatstraw_chemicals.subgroup(pretreatment_chemical_IDs)
...
pretreatment_sys.numerical_specification=BoundedNumericalSpecification(f_DSpret, 0.10, 0.70)    
...
fermentation_chemicals = wheatstraw_chemicals.subgroup(fermentation_chemical_IDs)
...
#CSL1 = Stream('CSL1',
#                CSL=211/1000*dryflow,#This is just a initialization
#                units='kg/hr',
#                price=price['CSL'])
#CSL2 = Stream('CSL2',
#                CSL=948/1000*dryflow,#This is just a initialization
#                units='kg/hr',
#                price=price['CSL'])
...
fermentation_sys.numerical_specification=BoundedNumericalSpecification(f_DSferm2, process_water2.F_mass/2, process_water2.F_mass*2)
...
#P402_1 = bst.Pump('P402_1', ins=H402-1)
#P402_1.BM = 3.1
...
P_sludge = 0.05/0.91/wheatstraw_chemicals.WWTsludge.MW
MW = np.array([wheatstraw_chemicals.CH4.MW, wheatstraw_chemicals.CO2.MW])
CH4_molcomp = 0.60
...
def anaerobic_rxn(reactant):
    MW = getattr(wheatstraw_chemicals, reactant).MW
...
#diluting_water = Stream('diluting_water', Water=dryflow*3, T=25+273.15)#F_mass is only a initialization
...
#H501 = bst.HXutility('H501', ins=diluting_water, T=25+273.15)
#M502 = bst.Mixer('M502', ins=(M501-0,diluting_water))
...
#TS_digestor = 0.05
#def update_diluting_water(dil_water_F_mass):
#    diluting_water.F_mass = dil_water_F_mass
#    for i in range(3): digestor_sys.simulate()
#    out1R501 = R501-1
#    out2R501 = R501-2
#    waterout_R501 = out1R501.imass['Water'] + out2R501.imass['Water']
#    totalout_R501 = out1R501.F_mass+out2R501.F_mass
#    TSout = (totalout_R501 - waterout_R501)/totalout_R501
#    return TSout - TS_digestor
#
#digestor_sys.numerical_specification = BoundedNumericalSpecification(update_diluting_water, dryflow*0.01, dryflow*0.5)    


# %% Waste water treatment
combustion = wheatstraw_chemicals.get_combustion_reactions()

def growth(reactant):
    f = wheatstraw_chemicals.WWTsludge.MW / getattr(wheatstraw_chemicals, reactant).MW
...
wheatstraw_sys.products.add(ash)
baghouse_bags = Stream(ID='Baghouse_bags', thermo=substance_thermo, flow=(1,), price=11.1)
wheatstraw_sys.feeds.add(lime)
wheatstraw_sys.feeds.add(boilerchems)
wheatstraw_sys.feeds.add(baghouse_bags)

for i in range(3): wheatstraw_sys.simulate()


ethanol_tea = WheatstrawTEA(
        system=wheatstraw_sys, IRR=0.06, duration=(2007, 2037),
...
wheatstraw_tea = bst.CombinedTEA([ethanol_tea, Area700], IRR=0.06)
wheatstraw_sys._TEA = wheatstraw_tea
ethanol.price = wheatstraw_tea.solve_price(ethanol, ethanol_tea)
ethanol.price = wheatstraw_tea.solve_price(ethanol, ethanol_tea)
